...
bool Matrix::is_symmetric() const {
    ...
    for (int i = 0; i < rows; ++i)
        for (int j = i + 1; j < cols; ++j)
            if (std::abs((*this)(i, j) - (*this)(j, i)) > COMPARISON_TOLERANCE)
                return false;
...
double Matrix::trace() const {
    ...
    if (!is_square())
        throw std::invalid_argument("Matrix trace requires a square matrix");
...
bool Matrix::operator==(const Matrix& other) const {
    ...
    for (size_t i = 0; i < data.size(); ++i)
        if (std::abs(data[i] - other.data[i]) > COMPARISON_TOLERANCE)
            return false;
...
std::tuple<Matrix, Matrix, Matrix, int> Matrix::_lu_internal() const {
    ...
    const int n = rows;
    Matrix u = *this;
    Matrix lower = Matrix::identity(n);
    ...
    for (int k = 0; k < n; ++k) {
        ...
        if (std::abs(u(k, k)) < PIVOT_TOLERANCE)
            continue;
...
std::optional<Matrix> Matrix::solve(const Matrix& b) const {
    ...
    if (!is_square() || b.rows != rows || b.cols != 1)
        return std::nullopt;
...
std::vector<double> Matrix::eigenvalues() const {
    ...
    if (!is_square())
        throw std::invalid_argument("Matrix eigenvalues require a square matrix");
