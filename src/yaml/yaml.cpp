		...
		while (!file.eof() && !faulty) {
			...
			if (dashPos != std::string::npos) {
				...
				if (colonPos != std::string::npos) {
					...
					Node* parentNode = parentsStack.top().first.get();
					// Lazy-init the list storage. The previous code called
					// getData<vector<string>>() on a node with null m_data,
					// which dereferences a null shared_ptr (UB) — it sometimes
					// "worked" and sometimes segfaulted depending on codegen.
					if (!parentNode->hasData()) {
						parentNode->setData(std::make_shared<std::vector<std::string>>());
					}
					parentNode->getData<std::vector<std::string>>().push_back(nodeID);
					continue;
				}
			}
