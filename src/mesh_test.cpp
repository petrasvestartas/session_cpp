...
MINI_TEST("Mesh", "Constructor") {
    ...
    MINI_CHECK(!sstr.empty());
    MINI_CHECK(!srepr.empty());
...
MINI_TEST("Mesh", "From Arrangement") {
    ...
    for (const std::pair<size_t, size_t>& edge : mesh.edges()) {
        const double line = *mesh.edge_attribute(edge, "line");
        horizontal += line == 0.0 ? 1 : 0;
        diagonal += line == 3.0 ? 1 : 0;
        sides += line >= 4.0 ? 1 : 0;
    }
...
MINI_TEST("Mesh", "Loft With Quads And Triangles") {
    ...
    for (size_t i = 0; i < panels.size(); i++) {
        ...
        for (const std::pair<const size_t, LoftFaceRole>& entry : panels[i].face_roles) {
            ...
            case LoftFaceRole::TopCap:
                ...
                break;
            case LoftFaceRole::BotCap:
                face_colors.push_back(Color::red());
                break;
            case LoftFaceRole::TriWall:
                face_colors.push_back(Color::yellow());
                break;
            case LoftFaceRole::QuadWall:
                face_colors.push_back(Color::grey());
                break;
            }
        }
    ...
    for (size_t i = 0; i < panels.size(); i++) {
        Point c = panels[i].mesh.centroid();
...
MINI_TEST("Mesh", "Attributes") {
    ...
    std::vector<size_t> vertices = mesh.vertices();
    ...
    std::vector<size_t> faces = mesh.faces();
    ...
    std::vector<std::pair<size_t, size_t>> edges = mesh.edges();
    ...
    std::vector<std::pair<size_t, size_t>> ne = mesh.naked_edges(true);
...
MINI_TEST("Mesh", "Vertex And Face Operations") {
    ...
    std::vector<size_t> fv3 = *mesh.face_vertices(3);
    std::vector<size_t> fv4 = *mesh.face_vertices(4);
    std::vector<size_t> fv5 = *mesh.face_vertices(5);
    ...
    fv3 = *mesh.face_vertices(3);
    fv4 = *mesh.face_vertices(4);
    fv5 = *mesh.face_vertices(5);
...
MINI_TEST("Mesh", "Connectivity Queries") {
    ...
    std::vector<size_t> v = mesh.vertices();
    std::vector<size_t> f = mesh.faces();
...
MINI_TEST("Mesh", "Geometric Properties") {
    ...
    for (size_t f : mesh.faces()) {
        std::optional<double> face_area = mesh.face_area(f);
    ...
    for (size_t f : mesh.faces()) {
        std::optional<Vector> fn = mesh.face_normal(f);
    ...
    for (const size_t f : mesh.faces()) {
        std::vector<size_t> fv = *mesh.face_vertices(f);

        for (const size_t v : fv) {
            std::optional<double> angle = mesh.vertex_angle_in_face(v, f);
    ...
    for (const size_t v : mesh.vertices()) {
        std::optional<Vector> vn = mesh.vertex_normal(v);
    ...
    MINI_CHECK(
        TOLERANCE.is_vector_close(vertex_normals[0], Vector(0.5773502691896258, 0.5773502691896258, 0.5773502691896258))
    );
    ...
    MINI_CHECK(TOLERANCE.is_vector_close(
        vertex_normals[3],
        Vector(0.5773502691896257, -0.5773502691896258, 0.5773502691896258)
    ));
    ...
    MINI_CHECK(TOLERANCE.is_vector_close(
        vertex_normals[6],
        Vector(0.5773502691896258, 0.5773502691896257, -0.5773502691896258)
    ));
    ...
    MINI_CHECK(TOLERANCE.is_vector_close(
        vertex_normals[9],
        Vector(-0.5773502691896258, 0.5773502691896258, 0.5773502691896257)
    ));
    ...
    for (const size_t v : mesh.vertices()) {
        std::optional<Vector> vnw = mesh.vertex_normal_weighted(v, NormalWeighting::Angle);
    ...
    MINI_CHECK(TOLERANCE.is_vector_close(
        ...
        Vector(0.5773502691896257, 0.5773502691896257, 0.5773502691896257)
    ));

    MINI_CHECK(
        TOLERANCE.is_vector_close(vertex_normals_weighted[1], Vector(0.0, 0.3568220897730899, 0.9341723589627158))
    );
    MINI_CHECK(
        TOLERANCE.is_vector_close(vertex_normals_weighted[2], Vector(0.0, -0.3568220897730899, 0.9341723589627158))
    );
    MINI_CHECK(TOLERANCE.is_vector_close(
        vertex_normals_weighted[3],
        Vector(0.5773502691896257, -0.5773502691896257, 0.5773502691896258)
    ));

    MINI_CHECK(
        TOLERANCE.is_vector_close(vertex_normals_weighted[4], Vector(0.9341723589627158, 0.0, 0.3568220897730899))
    );
    MINI_CHECK(
        TOLERANCE.is_vector_close(vertex_normals_weighted[5], Vector(0.9341723589627158, 0.0, -0.3568220897730899))
    );
    MINI_CHECK(TOLERANCE.is_vector_close(
        vertex_normals_weighted[6],
        Vector(0.5773502691896258, 0.5773502691896257, -0.5773502691896257)
    ));

    MINI_CHECK(
        TOLERANCE.is_vector_close(vertex_normals_weighted[7], Vector(0.3568220897730899, 0.9341723589627158, 0.0))
    );
    MINI_CHECK(
        TOLERANCE.is_vector_close(vertex_normals_weighted[8], Vector(-0.3568220897730899, 0.9341723589627158, 0.0))
    );
    MINI_CHECK(TOLERANCE.is_vector_close(
        vertex_normals_weighted[9],
        Vector(-0.5773502691896257, 0.5773502691896258, 0.5773502691896257)
    ));
    ...
    MINI_CHECK(
        TOLERANCE.is_vector_close(vertex_normals_weighted[11], Vector(0.0, -0.3568220897730899, -0.9341723589627158))
    );
    MINI_CHECK(
        TOLERANCE.is_vector_close(vertex_normals_weighted[12], Vector(0.0, 0.3568220897730899, -0.9341723589627158))
    );
    MINI_CHECK(TOLERANCE.is_vector_close(
    ...
    MINI_CHECK(
        TOLERANCE.is_vector_close(vertex_normals_weighted[15], Vector(-0.3568220897730900, -0.9341723589627158, 0.0))
    );
    MINI_CHECK(
        TOLERANCE.is_vector_close(vertex_normals_weighted[16], Vector(0.3568220897730899, -0.9341723589627158, 0.0))
    );
    MINI_CHECK(TOLERANCE.is_vector_close(
    ...
    MINI_CHECK(
        TOLERANCE.is_vector_close(vertex_normals_weighted[18], Vector(-0.9341723589627158, 0.0, -0.3568220897730899))
    );
    MINI_CHECK(
        TOLERANCE.is_vector_close(vertex_normals_weighted[19], Vector(-0.9341723589627158, 0.0, 0.3568220897730899))
    );
...
MINI_TEST("Mesh", "Section By Plane") {
    ...
    MINI_CHECK(above.empty());

    std::vector<Polyline> bottom = {
        Polyline::rectangle(Point(0.0, 0.0, 0.0), Vector(1.0, 0.0, 0.0), Vector(0.0, 1.0, 0.0), 4.0, 4.0),
        Polyline::rectangle(Point(1.0, 1.0, 0.0), Vector(1.0, 0.0, 0.0), Vector(0.0, 1.0, 0.0), 2.0, 2.0).reversed(),
    };
    std::vector<Polyline> top = {
...
MINI_TEST("Mesh", "Json Roundtrip") {
    ...
    std::string filename =
        (std::filesystem::path(__FILE__).parent_path().parent_path() / "serialization" / "test_mesh.json").string();
...
MINI_TEST("Mesh", "Protobuf Roundtrip") {
    ...
    std::string filename =
        (std::filesystem::path(__FILE__).parent_path().parent_path() / "serialization" / "test_mesh.bin").string();
...
MINI_TEST("Mesh", "Vertex Neighbors") {
    ...
    std::vector<size_t> n0v = *mesh.vertex_vertices(0);
...
MINI_TEST("Mesh", "Halfedge After Before") {
    ...
    std::optional<std::pair<size_t, size_t>> before = mesh.halfedge_before({0, 3});
...
MINI_TEST("Mesh", "Face Normal Unitized") {
    ...
    Vector nn = *mesh.face_normal_unitized(0, false);
...
MINI_TEST("Mesh", "Vertices Attribute Bulk") {
    ...
    std::vector<std::optional<double>> vals = mesh.vertices_attribute("is_support");
...
MINI_TEST("Mesh", "Vertices Where") {
    ...
    mesh.set_vertices_attribute("is_support", 1.0, &keys);
    std::vector<size_t> sup = mesh.vertices_where({{"is_support", 1.0}});
...
MINI_TEST("Mesh", "Faces Where") {
    ...
    std::vector<size_t> out = mesh.faces_where({{"tag", 7.0}});
...
MINI_TEST("Mesh", "Edges Where") {
    ...
    std::vector<std::pair<size_t, size_t>> out = mesh.edges_where({{"weight", 3.0}});
...
MINI_TEST("Mesh", "Vertices Where Predicate") {
    ...
    std::vector<size_t> big = mesh.vertices_where_predicate([](size_t, const std::map<std::string, double>& a) {
        auto it = a.find("load");
...
MINI_TEST("Mesh", "Faces Where Predicate") {
    ...
    std::vector<size_t> big = mesh.faces_where_predicate([](size_t, const std::map<std::string, double>& a) {
        auto it = a.find("area");
...
MINI_TEST("Mesh", "Edges Where Predicate") {
    ...
    mesh.update_default_edge_attributes({{"weight", 0.0}});
    mesh.set_edge_attribute({0, 1}, "weight", 5.0);
    std::vector<std::pair<size_t, size_t>> big =
        mesh.edges_where_predicate([](std::pair<size_t, size_t>, const std::map<std::string, double>& a) {
