...
double point_to_polygon_dist(double px, double py, const std::vector<Ring2>& polygon) {

  bool inside = false;
  double min_dist_sq = std::numeric_limits<double>::infinity();

  for (const Ring2& ring : polygon) {
...
PCell centroid_cell(const std::vector<Ring2>& polygon) {

  double area = 0.0;
  double cx = 0.0;
  double cy = 0.0;
  const Ring2& ring = polygon.at(0);
  const size_t len = ring.size();
...
std::array<double, 3> mapbox_polylabel(const std::vector<Ring2>& polygon, double precision) {
...
Polyline::Polyline(const Polyline& other)
    : name(other.name), _coords(other._coords), plane(other.plane), _plane_dirty(other._plane_dirty), width(other.width), dash(other.dash), linecolor(other.linecolor) {}
...
Polyline Polyline::from_sides(int sides, double radius, bool close) {
  ...
  for (int i = 0; i < sides; i++) {
    ...
    pts.emplace_back(radius * std::cos(angle), radius * std::sin(angle), 0.0);
  }
...
Polyline Polyline::quadratic_points(const Point& p0, const Point& p1, const Point& p2, int divisions) {
  ...
  for (int k = 0; k < n; k++) {
    ...
    const double t2 = t * t;
    pts.emplace_back(s2 * p0[0] + ts * p1[0] + t2 * p2[0], s2 * p0[1] + ts * p1[1] + t2 * p2[1], s2 * p0[2] + ts * p1[2] + t2 * p2[2]);
  }
...
void Polyline::merge_collinear(double tol) {
  ...
  const size_t max_pass = pts.size();
  bool changed = true;

  for (size_t pass = 0; pass < max_pass && changed && pts.size() >= 3; pass++) {
    changed = false;
    ...
    for (size_t i = 0; i < m; i++) {
      ...
      const Vector a = pts[i] - pts[p];
      const Vector b = pts[nx] - pts[i];
...
void Polyline::line_line_average(const Point& line0_start, const Point& line0_end, const Point& line1_start, const Point& line1_end, Point& output_start, Point& output_end) {
  output_start = Point((line0_start[0] + line1_start[0]) * 0.5, (line0_start[1] + line1_start[1]) * 0.5, (line0_start[2] + line1_start[2]) * 0.5);
  output_end = Point((line0_end[0] + line1_end[0]) * 0.5, (line0_end[1] + line1_end[1]) * 0.5, (line0_end[2] + line1_end[2]) * 0.5);
}
...
std::tuple<Point, Plane, double> Polyline::polylabel(const std::vector<Polyline>& polylines, double precision) {
  ...
  std::vector<Ring2> rings2d(polylines.size());
  ...
  for (int id : ids)
    polygon.push_back(std::move(rings2d[id]));
...
Polyline Polyline::jsonload(const nlohmann::json& data) {
  ...
  if (data.contains("coords")) {
    ...
    for (const nlohmann::json& pt_json : data["points"])
      polyline.add_point(Point::jsonload(pt_json));
  }
...
Polyline Polyline::pb_loads(const std::string& data) {
  ...
  Polyline pl = Polyline::from_coords(std::vector<double>(proto.coords().begin(), proto.coords().end()));
  ...
  if (proto.has_linecolor()) {
    const session_proto::Color& c = proto.linecolor();
...
Point Polyline::unproject(const Point& origin, const Vector& x_axis, const Vector& y_axis, double u, double v) {
  return Point(origin[0] + u * x_axis[0] + v * y_axis[0], origin[1] + u * x_axis[1] + v * y_axis[1], origin[2] + u * x_axis[2] + v * y_axis[2]);
}
