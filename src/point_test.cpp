...
namespace session_cpp {
    ...
    MINI_TEST("Point", "Constructor") {
        ...
        const std::string pstr = p.str();
        const std::string prepr = p.repr();

        const Point pcopy = p;
        const Point pother(1.0, 2.0, 3.0);
        ...
        const Point result_mul = p * 2.0;
        const Point result_div = p / 2.0;
        const Point result_add = p + Vector(1.0, 1.0, 1.0);
        const Point result_sub = p - Vector(1.0, 1.0, 1.0);
        const Vector result_diff = p - pother;
    ...
    MINI_TEST("Point", "Transformation") {
        ...
        const Point moved = p.transformed(xform);
        p.transform(xform);
    ...
    MINI_TEST("Point", "Json Roundtrip") {
        ...
        const std::string filename = "serialization/test_point.json";
    ...
    MINI_TEST("Point", "Protobuf Roundtrip") {
        ...
        const std::string guid = p.guid();
        const std::string filename = "serialization/test_point.bin";
        p.pb_dump(filename);

        const Point loaded = Point::pb_load(filename);
        const Point parsed = Point::pb_loads(p.pb_dumps());
        const Point converted = Point::from_proto(p.to_proto());
    ...
    MINI_TEST("Point", "Is Ccw") {
        ...
        const Point p2(0.05, 1.0, 0.0);
        const bool ccw = Point::is_ccw(p0, p1, p2);
        const bool cw = Point::is_ccw(p2, p1, p0);
    ...
    MINI_TEST("Point", "Distance") {
        ...
        const Point p0(0.0, 2.0, 1.0);
        const Point p1(1.0, 5.0, 3.0);
        const double d = Point::distance(p0, p1);
    ...
    MINI_TEST("Point", "Squared Distance") {
        ...
        const Point p0(0.0, 2.0, 1.0);
        const Point p1(1.0, 5.0, 3.0);
        const double d = Point::squared_distance(p0, p1);
    ...
    MINI_TEST("Point", "Interpolate") {
        ...
        const Point half = Point::lerp(a, b, 0.5);
        const std::vector<Point> inner = Point::interpolate(a, b, 3);
        const std::vector<Point> both = Point::interpolate(a, b, 3, 1);
        const std::vector<Point> start = Point::interpolate(a, b, 3, 2);
    ...
    MINI_TEST("Point", "Area") {
        ...
        const Point p3(0.0, 2.0, 0.0);
        const double area = Point::area({p0, p1, p2, p3});
    ...
    MINI_TEST("Point", "Centroid Quad") {
        ...
        const Point p3(0.0, 2.0, 1.0);
        const Point centroid = Point::centroid_quad({p0, p1, p2, p3});
    ...
    MINI_TEST("Point", "Centroid") {
        ...
        const Point p2(2.0, 2.0, 0.0);
        const Point p3(0.0, 2.0, 0.0);
        const Point centroid = Point::centroid({p0, p1, p2, p3});
    ...
    MINI_TEST("Point", "Dihedral Angle Deg") {
        ...
        const Point r(0.0, 1.0, 0.0);
        const Point s(0.0, 0.0, 1.0);
        const double angle = Point::dihedral_angle_deg(p, q, r, s);
