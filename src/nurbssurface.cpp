...
bool aabb_overlap_pad(const std::pair<std::array<double, 3>, std::array<double, 3>>& a, const std::pair<std::array<double, 3>, std::array<double, 3>>& b) {
...
void colors_to_proto(const std::vector<Color>& colors, google::protobuf::RepeatedPtrField<session_proto::Color>* field) {
    for (const Color& c : colors) {
...
NurbsSurface NurbsSurface::create(bool periodic_u, bool periodic_v, int degree_u, int degree_v, int cv_count_u, int cv_count_v, const std::vector<Point>& points) {
    if (degree_u < 1 || degree_v < 1)
        throw std::invalid_argument(fmt::format("NurbsSurface::create: degree must be >= 1, got degree_u={}, degree_v={}", degree_u, degree_v));
    if (cv_count_u < degree_u + 1)
        throw std::invalid_argument(fmt::format("NurbsSurface::create: cv_count_u ({}) must be >= degree_u+1 ({})", cv_count_u, degree_u + 1));
    if (cv_count_v < degree_v + 1)
        throw std::invalid_argument(fmt::format("NurbsSurface::create: cv_count_v ({}) must be >= degree_v+1 ({})", cv_count_v, degree_v + 1));
    const int expected = cv_count_u * cv_count_v;
    if (static_cast<int>(points.size()) != expected)
        throw std::invalid_argument(fmt::format("NurbsSurface::create: expected {} points ({}x{}), got {}", expected, cv_count_u, cv_count_v, static_cast<int>(points.size())));
...
bool NurbsSurface::create_raw(int dimension, bool is_rational, int order0, int order1, int cv_count0, int cv_count1, bool is_periodic_u, bool is_periodic_v, double nurbsknot_delta_u, double nurbsknot_delta_v) {
...
bool NurbsSurface::create_clamped_uniform(int dimension, int order0, int order1, int cv_count0, int cv_count1, double nurbsknot_delta0, double nurbsknot_delta1) {
    return create_raw(dimension, false, order0, order1, cv_count0, cv_count1, false, false, nurbsknot_delta0, nurbsknot_delta1);
}
...
std::tuple<std::vector<std::vector<Point>>, std::vector<std::vector<Vector>>, std::vector<std::vector<std::pair<double, double>>>> NurbsSurface::divide_by_count_points(int nu, int nv) const {
...
std::pair<std::vector<std::vector<Plane>>, std::vector<std::vector<std::pair<double, double>>>> NurbsSurface::divide_by_count_planes(int nu, int nv) const {
...
std::vector<Point> NurbsSurface::intersections_with_line(const Line& line) const {
    ...
    for (int a = 0; a <= nu; a++)
        for (int b = 0; b <= nv; b++) {
            ...
            const Vector r(p[0] - p0[0], p[1] - p0[1], p[2] - p0[2]);
...
std::vector<NurbsSurfaceTrimmed> NurbsSurface::split_by_plane(const Plane& plane, double tolerance) const {
    ...
    for (const auto& pair : Intersection::surface_plane_uv(*this, plane, tolerance))
...
std::vector<NurbsSurfaceTrimmed> NurbsSurface::split_by_surface(const NurbsSurface& cutter, double tolerance) const {
    std::vector<NurbsCurve> pcurves;
    for (const auto& triple : Intersection::surface_surface(*this, cutter, tolerance))
...
std::vector<NurbsSurfaceTrimmed> NurbsSurface::split_by_brep(const BRep& brep, double tolerance) const {
    const auto target_bb = surface_aabb(*this);
    std::vector<NurbsCurve> pcurves;
...
Mesh NurbsSurface::mesh_adaptive(double max_angle, double max_edge_length, double min_edge_length, double max_chord_height) const {
    if (m_mesh.number_of_vertices() == 0 && is_valid()) {
        RemeshNurbsSurfaceAdaptive mesher(*this);
        mesher.set_max_angle(max_angle).set_max_edge_length(max_edge_length).set_min_edge_length(min_edge_length).set_max_chord_height(max_chord_height);
        m_mesh = mesher.mesh();
    }
    return m_mesh;
}
...
NurbsSurface NurbsSurface::jsonload(const nlohmann::json& data) {
    NurbsSurface surface;
    if (!data.contains("dimension") || !data.contains("order_u") || !data.contains("order_v") || !data.contains("cv_count_u") || !data.contains("cv_count_v"))
        return surface;
    surface.create_raw(data["dimension"], data.value("is_rational", false), data["order_u"], data["order_v"], data["cv_count_u"], data["cv_count_v"]);
...
NurbsSurface NurbsSurface::pb_loads(const std::string& data) {
    ...
    NurbsSurface surface;
    surface.create_raw(proto.dimension(), proto.is_rational(), proto.order_u(), proto.order_v(), proto.cv_count_u(), proto.cv_count_v());
    if (!proto.guid().empty())
...
std::string NurbsSurface::str() const {
    return fmt::format("NurbsSurface(name={}, degree=({},{}), cvs=({},{}))", name, degree(0), degree(1), m_cv_count[0], m_cv_count[1]);
}
...
std::vector<std::vector<double>> NurbsSurface::basis_functions_derivatives(int dir, int span, double t, int deriv_order) const {
...
std::vector<Vector> NurbsSurface::rational_derivatives(const std::vector<std::vector<double>>& skl, int num_derivs) const {
    std::vector<Vector> result;
...
bool NurbsSurface::fundamental_forms(double u, double v, double& E, double& F, double& G, double& L, double& M, double& N) const {
...
Mesh NurbsSurface::mesh_planar() const {
    ...
    if (p00.distance(p01) < 1e-10) {
        const Vector e1(p10[0] - p00[0], p10[1] - p00[1], p10[2] - p00[2]);
        const Vector e2(p11[0] - p00[0], p11[1] - p00[1], p11[2] - p00[2]);
        normal = e1.cross(e2);
    } else {
