...
public:
    ...
    /// Apply this transform to a point (full homogeneous multiply; divides by w when projective)
    Point transform_point(const Point& p) const;

    /// Apply this transform to a vector (rotation/scale only — translation does not move directions)
    ...
    /// Return column-major layout as array of 4 columns, each with 4 rows
