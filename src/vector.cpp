...
Vector::Vector(const Vector& other)
    : _x(other._x), _y(other._y), _z(other._z), _magnitude(other._magnitude), _has_magnitude(other._has_magnitude), name(other.name) {}
...
// Accessors
// ═══════════════════════════════════════════════════════════════════════════
const std::string& Vector::guid() const {
...
Vector Vector::reflect(const Vector& plane_normal) const {
    ...
    return *this - plane_normal * (2.0 * d);
}
...
Vector Vector::average_normal(const std::vector<Point>& points) {
    ...
    const bool closed = (points.back() - points.front()).magnitude_squared() < 1e-10;
    const size_t n = closed ? points.size() - 1 : points.size();
    Vector normal(0.0, 0.0, 0.0);

    for (size_t i = 0; i < n; i++) {
        const size_t prev = (i + n - 1) % n;
        const size_t next = (i + 1) % n;
        const Vector a = points[i] - points[prev];
        const Vector b = points[next] - points[i];
        normal += a.cross(b);
    }
...
// Protobuf
// ═══════════════════════════════════════════════════════════════════════════
session_proto::Vector Vector::to_proto() const {
...
Vector Vector::from_proto(const session_proto::Vector& proto) {
