...
nlohmann::ordered_json ElementFeature::jsondump() const {
    ...
    for (const Polyline& o : outlines) outs.push_back(o.jsondump());
...
ElementFeature ElementFeature::jsonload(const nlohmann::json& data) {
    ...
    for (const auto& o : data.value("outlines", nlohmann::json::array()))
...
std::string ElementFeature::pb_dumps() const {
    ...
    for (const Polyline& o : outlines) proto.add_outlines()->ParseFromString(o.pb_dumps());
...
Element::Element(const Element& other)
    : name(other.name), _geometry(other._geometry), _geometry_ops(other._geometry_ops),
      _features(other._features), _insertion_vectors(other._insertion_vectors),
      _dimensions(other._dimensions), _element_type(other._element_type),
...
bool Element::has_geometry() const { return !std::holds_alternative<std::monostate>(_geometry); }
...
ElementGeometry Element::session_geometry(const Xform& xform) const {
    ...
    if (Mesh* mesh = std::get_if<Mesh>(&geo)) {
        *mesh = apply_geometry_ops(*mesh);
        if (!xform.is_identity()) mesh->transform(xform);
    } else if (BRep* brep = std::get_if<BRep>(&geo)) {
        if (!xform.is_identity()) brep->transform(xform);
    }
    return geo;
}
...
void Element::set_planes(std::vector<Plane> plns) { _planes = std::move(plns); }
...
Element Element::duplicate() const { return Element(*this); }

OBB Element::compute_aabb() { return obb_from_geometry(session_geometry(Xform::identity())); }

OBB Element::compute_obb() { return obb_from_geometry(session_geometry(Xform::identity())); }

Mesh Element::compute_collision_mesh() {
    ElementGeometry geo = session_geometry(Xform::identity());
    if (Mesh* mesh = std::get_if<Mesh>(&geo)) return *mesh;
    return Mesh();
}
...
Mesh Element::apply_geometry_ops(Mesh geo) const {
    for (const std::function<Mesh(Mesh)>& f : _geometry_ops) geo = f(geo);
    return geo;
}
...
std::vector<Point> Element::points_from_geometry(const ElementGeometry& geo) {
    ...
    if (const Mesh* mesh = std::get_if<Mesh>(&geo))
        for (const auto& [k, v] : mesh->vertex) points.push_back(v.position());
    if (const BRep* brep = std::get_if<BRep>(&geo)) points = brep->vertex_points();
    return points;
}

OBB Element::obb_from_geometry(const ElementGeometry& geo) {
    const std::vector<Point> points = points_from_geometry(geo);
    if (points.empty()) return OBB::from_point(Point(0, 0, 0), 0.0);
...
nlohmann::ordered_json Element::jsondump() const {
    ...
    if (const Mesh* mesh = std::get_if<Mesh>(&_geometry)) geo_data = mesh->jsondump();
    if (const BRep* brep = std::get_if<BRep>(&_geometry)) geo_data = brep->jsondump();
    ...
    for (const ElementFeature& f : _features) feats.push_back(f.jsondump());
    nlohmann::ordered_json ivs = nlohmann::ordered_json::array();
    for (const Vector& v : _insertion_vectors) ivs.push_back(v.jsondump());
    return nlohmann::ordered_json{
        {"dimensions", dims},
...
Element Element::jsonload(const nlohmann::json& data) {
    ...
    if (geo_type == "Mesh" && has_data) elem._geometry = Mesh::jsonload(data["geometry_data"]);
    if (geo_type == "BRep" && has_data) elem._geometry = BRep::jsonload(data["geometry_data"]);
    const std::string g = data.value("guid", std::string());
    if (!g.empty()) elem.guid() = g;
    ...
    for (const auto& f : data.value("features", nlohmann::json::array()))
        elem._features.push_back(ElementFeature::jsonload(f));
    for (const auto& v : data.value("insertion_vectors", nlohmann::json::array()))
...
Element Element::file_json_loads(const std::string& s) { return jsonload(nlohmann::ordered_json::parse(s)); }
...
std::string Element::pb_dumps() const {
    ...
    if (const Mesh* mesh = std::get_if<Mesh>(&_geometry)) proto.set_geometry_data(mesh->pb_dumps());
    if (const BRep* brep = std::get_if<BRep>(&_geometry)) proto.set_geometry_data(brep->pb_dumps());
    ...
    for (const ElementFeature& f : _features) proto.add_features()->ParseFromString(f.pb_dumps());
    return proto.SerializeAsString();
}
...
Element Element::pb_loads(const std::string& data) {
    ...
    if (proto.geometry_type() == "Mesh" && has_data) elem._geometry = Mesh::pb_loads(proto.geometry_data());
    if (proto.geometry_type() == "BRep" && has_data) elem._geometry = BRep::pb_loads(proto.geometry_data());
    ...
    for (int i = 0; i + 2 < proto.insertion_vectors_size(); i += 3)
        elem._insertion_vectors.push_back(Vector(
            proto.insertion_vectors(i),
    ...
    for (const auto& f : proto.features()) elem._features.push_back(ElementFeature::pb_loads(f.SerializeAsString()));
    return elem;
}
...
static std::shared_ptr<Element> build_registered(const std::string& type_name, const std::string& data) {
    ...
    if (it == element_registry().end()) return nullptr;
    try {
...
bool Element::is_registered(const std::string& type_name) { return element_registry().count(type_name) > 0; }
...
std::shared_ptr<Element> Element::pb_loads_polymorphic(const std::string& data) {
    session_proto::Element proto;
    proto.ParseFromString(data);
    if (std::shared_ptr<Element> derived = build_registered(proto.element_type(), data)) return derived;
...
std::shared_ptr<Element> Element::file_json_loads_polymorphic(const std::string& s) {
    ...
    if (std::shared_ptr<Element> derived = build_registered(base.element_type_name(), base.pb_dumps())) return derived;
    return std::make_shared<Element>(std::move(base));
}

std::string Element::str() const { return fmt::format("Element({}, {})", name, geometry_type_name()); }

std::string Element::repr() const { return fmt::format("Element({}, {}, {})", guid(), name, geometry_type_name()); }
