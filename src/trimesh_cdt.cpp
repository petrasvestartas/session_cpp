 ...
 * Self-contained (no external Clipper2 dependency).
...
static const int undefined_error_i = 0;
...
{
    int asc, des;
    if (v->edges[0]->kind == EdgeKind::ascend) { asc = 0; des = 1; }
    else { des = 0; asc = 1; }
    ...
    const Point64& segPt1, const Point64& segPt2)
{
    ...
    else if (qNum > Sqr(dx) + Sqr(dy)) return DistanceSqr(pt, segPt2);
    else return Sqr(ax * dy - dx * ay) / (dx * dx + dy * dy);
}
    ...
    const std::vector<std::vector<std::pair<double,double>>>& holes_2d)
{
    ...
    // Point64 → flat index map (first occurrence wins)
    ...
    // Build Path64 (strip closing duplicate)
