...
KDTree::KDTree(std::vector<Point> points) : _points(std::move(points)) {
    if (!_points.empty()) {
...
double KDTree::dist_sq(const Point& a, const Point& b) {
    double dx = a[0]-b[0], dy = a[1]-b[1], dz = a[2]-b[2];
    return dx*dx + dy*dy + dz*dz;
}

std::unique_ptr<KDTree::Node> KDTree::build(const std::vector<Point>& pts, std::vector<int>& indices, int depth) {
...
void KDTree::nearest_1(const Node* node, const std::vector<Point>& pts, const Point& q, int& best_idx, double& best_d2) {
    if (!node) return;
...
std::pair<int, double> KDTree::nearest(const Point& query) const {
    int best_idx = 0;
...
void KDTree::nearest_k_rec(const Node* node, const std::vector<Point>& pts, const Point& q, int k, std::vector<std::pair<double, int>>& heap) {
    if (!node) return;
    double d = dist_sq(q, pts[node->idx]);
    if ((int)heap.size() < k) {
...
std::vector<std::pair<int, double>> KDTree::nearest_k(const Point& query, int k) const {
...
void KDTree::radius_rec(const Node* node, const std::vector<Point>& pts, const Point& q, double r2, std::vector<std::pair<int, double>>& result) {
    if (!node) return;
    double d = dist_sq(q, pts[node->idx]);
...
std::vector<std::pair<int, double>> KDTree::radius_search(const Point& query, double radius) const {
    std::vector<std::pair<int, double>> result;
