...
static bool are_finite(const std::vector<double>& values, std::size_t count) {
    ...
    for (auto value = values.begin(); value != values.begin() + count; value++)
...
std::vector<double> make_clamped_uniform(int order, int cv_count, double delta) {
    ...
    if (kc == 0) return std::vector<double>();
...
bool clamp(int order, int cv_count, std::vector<double>& nurbsknot, int end) {
    if (order < 2 || cv_count < order || end < 0 || end > 2) return false;
    ...
    if (end == 0 || end == 2) {
        ...
        for (int i = 0; i < order - 2; i++) nurbsknot[i] = clamp_value;
    }

    if (end == 1 || end == 2) {
        const double clamp_value = nurbsknot[cv_count - 1];
        for (int i = cv_count; i < kc; i++) nurbsknot[i] = clamp_value;
    }
...
bool is_valid(int order, int cv_count, const std::vector<double>& nurbsknot) {
    ...
    for (int i = 1; i < kc; i++)
        if (nurbsknot[i] < nurbsknot[i - 1]) return false;

    for (int i = 0; i < kc - order + 1; i++)
        if (nurbsknot[i] >= nurbsknot[i + order - 1]) return false;
...
bool is_clamped(int order, int cv_count, const std::vector<double>& nurbsknot, int end) {
    ...
    if (end == 0 || end == 2) {
        if (mult > kc) return false;
        const double start_value = nurbsknot[0];
        for (int i = 1; i < mult; i++)
            if (std::fabs(nurbsknot[i] - start_value) > tol) return false;
    }
    ...
    if (end == 1 || end == 2) {
        ...
        for (int i = 1; i < mult; i++)
            if (std::fabs(nurbsknot[kc - 1 - i] - end_value) > tol) return false;
    }
...
bool is_periodic(int order, int cv_count, const std::vector<double>& nurbsknot) {
    ...
    const double tol = KNOT_TOLERANCE;
    for (int i = 2; i < kc; i++)
        if (std::fabs((nurbsknot[i] - nurbsknot[i - 1]) - delta) > tol) return false;
...
std::pair<double, double> get_domain(int order, int cv_count,
    ...
    if (order < 2 || cv_count < order) return std::make_pair(0.0, 0.0);

    const int kc = nurbsknot_count(order, cv_count);
    if (kc == 0 || nurbsknot.size() < static_cast<std::size_t>(kc)) return std::make_pair(0.0, 0.0);
    const double start = nurbsknot[order - 2];
    const double end = nurbsknot[cv_count - 1];
    if (!std::isfinite(start) || !std::isfinite(end)) return std::make_pair(0.0, 0.0);
...
bool set_domain(int order, int cv_count, std::vector<double>& nurbsknot, double t0, double t1) {
    ...
    if (old_t1 <= old_t0) return false;

    const double scale = (t1 - t0) / (old_t1 - old_t0);
    for (int i = 0; i < kc; i++) nurbsknot[i] = t0 + (nurbsknot[i] - old_t0) * scale;
...
bool reverse(int order, int cv_count, std::vector<double>& nurbsknot) {
    ...
    for (int i = 0; i < kc; i++) nurbsknot[i] = t0 + t1 - nurbsknot[i];
...
int span_count(int order, int cv_count, const std::vector<double>& nurbsknot) {
    ...
    for (int i = 0; i < cv_count - order + 1; i++)
        if (nurbsknot[i + d - 1] < nurbsknot[i + d]) count++;

    return count;
}

int find_span(int order, int cv_count, const std::vector<double>& nurbsknot, double t, int, int) {
    if (order < 2 || cv_count < order || !std::isfinite(t)) return 0;

    const int kc = nurbsknot_count(order, cv_count);
    if (kc == 0 || nurbsknot.size() != static_cast<std::size_t>(kc)) return 0;
    ...
    if (!std::isfinite(start) || !std::isfinite(end)) return 0;

    if (t <= start) return 0;
    if (t >= end) return span_len - 2;
    ...
    while (high > low + 1) {
        ...
        if (!std::isfinite(mid_value)) return 0;
        if (t < mid_value)
            high = mid;
        else
            low = mid;
...
std::vector<double> get_greville_abcissae(int order, int cv_count,
    ...
    if (order < 2 || cv_count < order) return std::vector<double>();
    ...
    for (int i = 0; i < count; i++) {
        double sum = 0.0;
        for (int j = 0; j < d; j++) sum += nurbsknot[i + j];
        g[i] = sum / d;
    }
...
bool solve_tridiagonal(int dim, int n, std::vector<double>& lower, std::vector<double>& diag,
    ...
    for (int d = 0; d < dim; d++) d_star[d] = rhs[d] / diag[0];
...
std::vector<double> compute_parameters(const double* points, int point_count, int dim,
    ...
    if (point_count < 1 || dim < 1) return std::vector<double>();
    const std::size_t value_count =
        static_cast<std::size_t>(point_count) * static_cast<std::size_t>(dim);
    if (!are_finite(points, value_count)) return std::vector<double>();
...
std::vector<double> build_interp_nurbsknots(const std::vector<double>& params, int degree) {
    ...
    for (int i = 1; i < n - 1; i++) nurbsknots[order - 2 + i] = params[i];

    for (int i = 0; i < order - 1; i++) nurbsknots[kc - 1 - i] = t_max;
...
std::vector<double> eval_basis(int order, const std::vector<double>& nurbsknot, int span,
                               double t) {
    if (order < 1 || span < 0 || !std::isfinite(t)) return std::vector<double>();
    ...
    if (first > std::numeric_limits<std::size_t>::max() - width) return std::vector<double>();
    const std::size_t end = first + width;
    if (nurbsknot.size() < end) return std::vector<double>();
    for (std::size_t i = first; i < end; i++)
        if (!std::isfinite(nurbsknot[i])) return std::vector<double>();
...
static double turn_angle(const double* points, int dim, int prev, int i, int next) {
    ...
    if (len1 <= PIVOT_TOLERANCE || len2 <= PIVOT_TOLERANCE) return 0.0;
...
std::vector<double> build_fitted_nurbsknots_adaptive(const std::vector<double>& params,
    ...
    if (m < 2 || dim < 1 || num_cvs <= degree || degree < 1 || !std::isfinite(scale) ||
        ...
        return std::vector<double>();
    if (m < 3 || !points) {
        if (m < num_cvs - degree) return std::vector<double>();
    ...
    for (int i = 1; i < m - 1; i++) turn[i] = turn_angle(points, dim, i - 1, i, i + 1);
...
std::vector<double> build_fitted_nurbsknots_periodic_adaptive(const std::vector<double>& params,
    ...
    if (kc64 > std::numeric_limits<int>::max()) return std::vector<double>();
    ...
    if (!std::isfinite(period) || period <= 0.0) return std::vector<double>();
    if (n < 3 || !points) {
...
bool solve_banded_spd(int dim, int n, int half_bw, std::vector<double>& band,
    ...
    if (dim < 1 || n < 1 || half_bw < 0 || half_bw == std::numeric_limits<int>::max()) return false;
    const std::size_t band_count =
        static_cast<std::size_t>(n) * (static_cast<std::size_t>(half_bw) + 1);
    const std::size_t rhs_count = static_cast<std::size_t>(n) * static_cast<std::size_t>(dim);
    if (!are_finite(band, band_count) || !are_finite(rhs, rhs_count)) return false;
    ...
    for (int i = 0; i < n; i++) {
        for (int j = std::max(0, i - half_bw); j <= i; j++) {
            ...
            if (i == j) {
                ...
                if (val <= POSITIVE_DEFINITE_TOLERANCE) return false;
