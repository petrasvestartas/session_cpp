...
struct Quadtree {
    ...
    std::map<Key, Corner> corners;
    std::map<int64_t, std::vector<int64_t>> rows;
    std::map<int64_t, std::vector<int64_t>> cols;
    Mesh mesh;
    ...
    Quadtree(const NurbsSurface& surface, double norm_tol, double chord_tol, double max_edge, double min_edge)
        : s(surface), usp(surface.get_span_vector(0)), vsp(surface.get_span_vector(1)), closed{surface.is_closed(0), surface.is_closed(1)}, norm_tol(norm_tol), chord_tol(chord_tol), max_edge(max_edge), min_edge(min_edge) {}
};
...
Corner sample(const NurbsSurface& s, double u, double v) {
    ...
    if (derivatives.size() < 3) return c;
    const Vector n = derivatives[2].cross(derivatives[1]);
    const double length = norm(n);
    if (length > 1e-10) c.n = n / length;
    return c;
}

/// Leaf cell over [u0, u1] x [v0, v1] from its sampled corners SW SE NE NW, centre sampled here
Node make_node(const NurbsSurface& s, double u0, double v0, double u1, double v1, const std::array<Corner, 4>& corners, int depth) {
...
bool normals_turn(const Corner& a, const Corner& b, double norm_tol) {
    if (a.n.magnitude_squared() <= 1e-20 || b.n.magnitude_squared() <= 1e-20) return false;
...
bool too_short(const Node& p, double min_edge) {
    if (min_edge <= 0.0) return false;
    double longest = 0.0;
    for (int i = 0; i < 4; ++i) longest = std::max(longest, dist2(p.c[i].p, p.c[(i + 1) % 4].p));
...
bool twisted(const Node& p, double chord_tol) {
    for (int i = 0; i < 4; ++i)
        if (dist2(p.c[i].p, p.c[(i + 1) % 4].p) < chord_tol * chord_tol) return false;
    const double twist_tol2 = 4.0 * chord_tol * chord_tol;
    return dist2(p.c[4].p, midpoint(p.c[0].p, p.c[2].p)) > twist_tol2 || dist2(p.c[4].p, midpoint(p.c[1].p, p.c[3].p)) > twist_tol2;
}
...
std::pair<bool, bool> curved(const NurbsSurface& s, const Node& p, double chord_tol) {
    ...
    if (d.size() < 6) return {false, false};
    const Vector su = d[3];
    const Vector sv = d[1];
    ...
    if (length <= 1e-10 || su2 <= 1e-20 || sv2 <= 1e-20) return {false, false};
...
void subdivide(Quadtree& q, int root) {
    ...
    while (top > 0) {
        ...
        const Node p = q.nodes[idx];
        if (p.depth >= MAX_DEPTH || too_short(p, q.min_edge)) continue;
        ...
        for (int i = count - 1; i >= 0; --i) stack[top++] = first + i;
    }
}
...
void build(Quadtree& q) {
    ...
    for (int i = 0; i + 1 < nu; ++i)
        for (int j = 0; j + 1 < nv; ++j) {
            ...
            q.nodes.push_back(make_node(q.s, q.usp[i], q.vsp[j], q.usp[i + 1], q.vsp[j + 1], {grid[i * nv + j], grid[(i + 1) * nv + j], grid[(i + 1) * nv + j + 1], grid[i * nv + j + 1]}, 0));
            subdivide(q, root);
        }
}
...
void index_leaves(Quadtree& q) {
    ...
    for (auto& [key, line] : q.rows) sort_unique(line);
    for (auto& [key, line] : q.cols) sort_unique(line);
}
...
std::vector<double> between(const std::map<int64_t, std::vector<int64_t>>& lines, int64_t line, double t0, double t1) {
    ...
    if (found == lines.end()) return result;
    ...
    for (auto k = first; k != last; ++k) result.push_back(*k / KEY_SCALE);
    if (t0 > t1) std::reverse(result.begin(), result.end());
    return result;
}
...
size_t vertex_at(Quadtree& q, double u, double v) {
    if (q.south && std::abs(v - q.vsp.front()) < 1e-10) return *q.south;
    if (q.north && std::abs(v - q.vsp.back()) < 1e-10) return *q.north;
    ...
    if (found != q.keys.end()) return found->second;
    if (!q.corners.count(key)) q.corners[key] = sample(q.s, uw, vw);
...
std::vector<size_t> leaf_polygon(Quadtree& q, const Node& nd) {
    ...
    for (double u : row_mids(q, nd.u0, nd.u1, nd.v0)) poly.push_back(vertex_at(q, u, nd.v0));
    poly.push_back(vertex_at(q, nd.u1, nd.v0));
    for (double v : col_mids(q, nd.u1, nd.v0, nd.v1)) poly.push_back(vertex_at(q, nd.u1, v));
    poly.push_back(vertex_at(q, nd.u1, nd.v1));
    for (double u : row_mids(q, nd.u1, nd.u0, nd.v1)) poly.push_back(vertex_at(q, u, nd.v1));
    poly.push_back(vertex_at(q, nd.u0, nd.v1));
    for (double v : col_mids(q, nd.u0, nd.v1, nd.v0)) poly.push_back(vertex_at(q, nd.u0, v));
...
void add_leaf_faces(Quadtree& q, const Node& nd) {
    ...
    for (int i = 0; i < n; ++i) {
        const int j = (i + 1) % n;
        if (poly[i] != poly[j] && poly[i] != centre && poly[j] != centre) q.mesh.add_face({poly[i], poly[j], centre});
    }
}
...
Mesh RemeshNurbsSurfaceAdaptive::mesh() const {
    ...
    if (m_surface.is_singular(0)) q.south = q.mesh.add_vertex(m_surface.point_at(q.usp.front(), q.vsp.front()));
    if (m_surface.is_singular(2)) q.north = q.mesh.add_vertex(m_surface.point_at(q.usp.front(), q.vsp.back()));
    for (const adaptive::Node& nd : q.nodes)
        if (nd.leaf) adaptive::add_leaf_faces(q, nd);
