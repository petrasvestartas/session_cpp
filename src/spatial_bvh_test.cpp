...
MINI_TEST("SpatialBVH", "Constructor") {
    ...
    const std::vector<int> n = bvh.nearest_neighbors(0, boxes, 1.5);
...
MINI_TEST("SpatialBVH", "Expand Bits") {
    ...
    const uint32_t result = expand_bits(1023);
...
MINI_TEST("SpatialBVH", "Morton Code Corners") {
    ...
    const uint32_t code_max = calculate_morton_code(50.0, 50.0, 50.0, 100.0);
...
MINI_TEST("SpatialBVH", "Morton Code Spatial Locality") {
    ...
    const uint32_t code3 = calculate_morton_code(-40.0, -40.0, -40.0);
    const uint32_t diff_nearby = code1 > code2 ? code1 - code2 : code2 - code1;
    const uint32_t diff_far = code1 > code3 ? code1 - code3 : code3 - code1;
...
MINI_TEST("SpatialBVH", "Aabb Intersect") {
    ...
    const OBB bbox2 = OBB::from_aabb(AABB(0.5, 0.0, 0.0, 1.0, 1.0, 1.0));
    const OBB bbox3 = OBB::from_aabb(AABB(10.0, 0.0, 0.0, 1.0, 1.0, 1.0));

    MINI_CHECK(bvh.aabb_intersect(bbox1, bbox2));
    MINI_CHECK(!bvh.aabb_intersect(bbox1, bbox3));
    MINI_CHECK(bvh.aabb_intersect(AABB(0.0, 0.0, 0.0, 1.0, 1.0, 1.0), AABB(1.5, 0.0, 0.0, 1.0, 1.0, 1.0)));
}
...
MINI_TEST("SpatialBVH", "Check All Collisions") {
    ...
    const std::tuple<std::vector<std::pair<int, int>>, std::vector<int>, int> result = bvh.check_all_collisions(boxes);
    const std::vector<std::pair<int, int>>& collisions = std::get<0>(result);
    const std::vector<int>& colliding_indices = std::get<1>(result);
...
MINI_TEST("SpatialBVH", "Nearest Neighbors") {
    ...
    const SpatialBVH bvh = SpatialBVH::from_boxes(boxes, 100.0);
    const std::vector<int> n0 = bvh.nearest_neighbors(0, boxes, 1.2);

    MINI_CHECK(n0.size() == 1);
    MINI_CHECK(n0[0] == 1);

    const std::vector<int> n2 = bvh.nearest_neighbors(2, boxes, 1.2);

    MINI_CHECK(n2.empty());

    const std::vector<int> n2_wide = bvh.nearest_neighbors(2, boxes, 10.0);
...
MINI_TEST("SpatialBVH", "Merge Aabb") {
    ...
    const SpatialBVH bvh(100.0);
    const OBB bbox1 = OBB::from_aabb(AABB(0.0, 0.0, 0.0, 1.0, 1.0, 1.0));
    const OBB bbox2 = OBB::from_aabb(AABB(5.0, 0.0, 0.0, 1.0, 1.0, 1.0));
    const OBB merged = bvh.merge_aabb(bbox1, bbox2);
...
MINI_TEST("SpatialBVH", "Fixed 100 Boxes") {
    ...
    for (const std::array<double, 6>& corner : corners) {
        const Point lo(corner[0], corner[1], corner[2]);
        const Point hi(corner[3], corner[4], corner[5]);
        boxes.push_back(OBB::from_aabb(AABB::from_points({lo, hi})));
    }

    SpatialBVH bvh = SpatialBVH::from_boxes(boxes, 100.0);
    std::vector<std::pair<int, int>> pairs = std::get<0>(bvh.check_all_collisions(boxes));
    ...
    for (const std::pair<int, int>& pair : pairs) {
        MINI_CHECK(pair.first < 100);
        MINI_CHECK(pair.second < 100);
        MINI_CHECK(pair.first < pair.second);
    }
}
...
MINI_TEST("SpatialBVH", "Query Aabb") {
    ...
    const SpatialBVH bvh = SpatialBVH::from_boxes(boxes, 100.0);
    const std::vector<int> hits = bvh.query_aabb(OBB::from_aabb(AABB(0.0, 0.0, 0.0, 0.5, 0.5, 0.5)));
    ...
    const std::vector<int> hits_all = bvh.query_aabb(AABB(2.5, 2.5, 0.0, 5.0, 5.0, 2.0));
...
MINI_TEST("SpatialBVH", "Build From Boxes") {
    ...
    MINI_CHECK(pairs[0] == std::make_pair(0, 1));
}
...
MINI_TEST("SpatialBVH", "Build From Aabbs") {
    ...
    const std::vector<int> hits = bvh.query_aabb(AABB(0.0, 0.0, 0.0, 2.0, 2.0, 2.0));
...
MINI_TEST("SpatialBVH", "Check All Collisions Guids") {
    ...
    SpatialBVH bvh;
    bvh.build_with_guids(boxes_with_guids);
    const std::vector<std::pair<std::string, std::string>> guid_pairs = bvh.check_all_collisions_guids(boxes);

    MINI_CHECK(guid_pairs.size() == 1);
    MINI_CHECK(guid_pairs[0].first == "a");
    MINI_CHECK(guid_pairs[0].second == "b");
}
...
MINI_TEST("SpatialBVH", "Find Collisions") {
    ...
    const SpatialBVH bvh = SpatialBVH::from_boxes(boxes, 100.0);
    const std::pair<std::vector<int>, int> found0 = bvh.find_collisions(0, boxes[0], boxes);
    const std::pair<std::vector<int>, int> found2 = bvh.find_collisions(2, boxes[2], boxes);

    MINI_CHECK(found0.first == std::vector<int>{1});
    MINI_CHECK(found0.second > 0);
    MINI_CHECK(found2.first.empty());
}
...
MINI_TEST("SpatialBVH", "Ray Cast") {
    ...
    const SpatialBVH bvh = SpatialBVH::from_boxes(boxes, 100.0);
    std::vector<int> hits;
    const bool hit = bvh.ray_cast(Point(-10.0, 0.0, 0.0), Vector(1.0, 0.0, 0.0), hits);
    ...
    const bool miss = bvh.ray_cast(Point(-10.0, 20.0, 0.0), Vector(1.0, 0.0, 0.0), hits);
