  ...
  for (std::vector<NurbsCurve> &chain : chains) {
    ...
    for (size_t ci = 1; ci < chain.size(); ci++) {
      ...
      if (rational) {
        ...
        const double w_start = c.weight(0);
      ...
      if (stride <= 0 || cvdim <= 0 || c.m_order != joined.m_order || c.m_cv_stride != stride || c.cv_size() != cvdim || static_cast<int>(joined.m_cv.size()) < last + cvdim || static_cast<int>(c.m_cv.size()) < c.m_cv_count * stride || static_cast<int>(c.m_cv.size()) <= stride || static_cast<int>(c.m_nurbsknot.size()) != c.m_cv_count + c.m_order - 2)
        continue;
    ...
    if (static_cast<int>(joined.m_cv.size()) < (joined.m_cv_count - 1) * joined.m_cv_stride + joined.cv_size() || static_cast<int>(joined.m_nurbsknot.size()) != joined.m_cv_count + joined.m_order - 2) {
      for (NurbsCurve &c : chain)
        result.push_back(c);
      continue;
    }
...
double NurbsCurve::greville_abcissa(int cv_index) const {
  ...
  const double tol = (k1 - k0) * 1.490116119385e-8;
  double g = 0.0;
  for (int i = 0; i < p; i++)
    g += nurbsknot[i];
  g /= static_cast<double>(p);
  if (std::fabs(2.0 * k - (k0 + k1)) <= tol && std::fabs(g - k) <= (std::fabs(g) * 1.490116119385e-8 + tol))
    g = k;
  return g;
}
...
bool NurbsCurve::to_polyline_adaptive(std::vector<Point> &points, std::vector<double> *params, double angle_tolerance, double min_edge_length, double max_edge_length) const {
  ...
  std::sort(samples.begin(), samples.end(), [](const std::pair<double, Point> &a, const std::pair<double, Point> &b) { return a.first < b.first; });
...
Plane NurbsCurve::perpendicular_plane_at(double t, bool normalized) const {
  ...
  const std::vector<Vector> derivs0 = evaluate(t0, 2);
  ...
  const Vector r0 = N0_unnorm / N0_mag;
  const Point origin = point_at(param);
...
bool NurbsCurve::trim(double t0, double t1) {
  ...
  const double stol = (std::abs(d0) + std::abs(d1) + std::abs(d1 - d0)) * std::sqrt(std::numeric_limits<double>::epsilon());
  for (double k : m_nurbsknot) {
  ...
  if (is_periodic()) {
    ...
    if (sc + 2 * p > kc) {
      ...
      if (nurbsknot_index >= p && nurbsknot_index <= kc - p) {
        const double k0 = m_nurbsknot[nurbsknot_index - 1];
        const double k1 = m_nurbsknot[nurbsknot_index];
        const double d0 = t - k0;
        const double d1 = k1 - t;
...
bool NurbsCurve::evaluate_nurbs_de_boor(int cv_dim, int order, int cv_stride, double *cv, const double *nurbsknots, int side, double t) {
  ...
  if (side < 0) {
    ...
    if (fully_multiple) {
      ...
      while (--k) {
        ...
        int i = k;
        while (i--) {
          const double alpha1 = delta_t[di++] / (*k1-- - *k0--);
          const double alpha0 = 1.0 - alpha1;
          cv0 -= cv_inc;
          cv1 -= cv_inc;
          int j = cv_dim;
  ...
  if (fully_multiple) {
    ...
    while (--k) {
      ...
      int i = k;
      while (i--) {
        const double alpha0 = delta_t[di++] / (*k1++ - *k0++);
        const double alpha1 = 1.0 - alpha0;
        int j = cv_dim;
        while (j--) {
          *cv0 = *cv0 * alpha0 + *cv1 * alpha1;
