...
std::vector<int> span_subs(
    ...
    for (size_t i = 0; i + 1 < sp.size(); ++i) {
        ...
        if (dev > chord_tol)
            subs[i] = std::max(subs[i], std::clamp((int)std::ceil(std::sqrt(dev / chord_tol)), 2, MAX_SUBS));

        if (degree > 1)
            subs[i] = std::max(subs[i], 2);
    }
...
void balance_subs(
    const NurbsSurface& s,
    const std::vector<double>& usp,
    const std::vector<double>& vsp,
    std::vector<int>& u_subs,
    std::vector<int>& v_subs
) {
    ...
    if (ratio > 2.0 && s.degree(0) > 1) {
        ...
        for (int& sub : u_subs)
            sub = std::min(MAX_SUBS, (int)std::ceil(sub * scale));
...
void make_odd(std::vector<int>& subs) {
    ...
    if (total % 2 == 0)
        *std::max_element(subs.begin(), subs.end()) += 1;
}
...
void add_faces(
    ...
    if (north)
        for (int i = 0; i < nu_faces; ++i)
            mesh.add_face({grid[i * nv + nv - 1], grid[((i + 1) % nu) * nv + nv - 1], *north});
}
    ...
    for (auto& [key, vd] : mesh.vertex) {
        ...
        if (key != south && key != north) {
            ...
            if (std::isfinite(length) && length > 0.0)
                n = raw.dot(n) < 0.0 ? -raw / length : raw / length;
        }
...
unsigned crease_flags(const NurbsSurface& s, double u, double v) {
    ...
    for (int dir = 0; dir < 2; ++dir) {
        ...
        if (std::isfinite(dot) && dot < 1.0 - 64.0 * std::numeric_limits<double>::epsilon())
            flags |= 1u << dir;
    }
...
unsigned crease_side(const double center[2], double uv[2], unsigned flags) {
    ...
    for (int dir = 0; dir < 2; ++dir) {
        ...
        uv[dir] = std::nextafter(
            uv[dir],
            high ? std::numeric_limits<double>::infinity() : -std::numeric_limits<double>::infinity()
        );
    }
...
Mesh RemeshNurbsSurfaceGrid::from_u_v_q(
    const NurbsSurface& s,
    int max_u,
    int max_v,
    double max_angle_deg,
    ...
    if (sing_v0)
        south = add_vertex_uv(s, mesh, us[0], vs[0]);
