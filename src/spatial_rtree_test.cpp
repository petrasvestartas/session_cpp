...
namespace session_cpp {
    ...
    MINI_TEST("SpatialRTree", "Constructor") {
        ...
        const double max0[3] = {1, 1, 1};
        const double min1[3] = {5, 0, 0};
        const double max1[3] = {6, 1, 1};
        const double min2[3] = {10, 0, 0};
        const double max2[3] = {11, 1, 1};

        tree.insert(min0, max0, 0);
        tree.insert(min1, max1, 1);
        tree.insert(min2, max2, 2);

        int found = -1;
        const int hits = tree.search(min0, max0, [&](int data) -> bool {
            found = data;
            return true;
        });
    ...
    MINI_TEST("SpatialRTree", "Insert Multiple") {
        ...
        const double min0[3] = {0, 0, 0};
        const double max0[3] = {1, 1, 1};
        const double min1[3] = {2, 2, 2};
        const double max1[3] = {3, 3, 3};
        const double min2[3] = {4, 4, 4};
        const double max2[3] = {5, 5, 5};
    ...
    MINI_TEST("SpatialRTree", "Search Hit") {
        ...
        SpatialRTree<int, double, 3> tree;
        const double min0[3] = {0, 0, 0};
        const double max0[3] = {2, 2, 2};
        const double qmin[3] = {1, 1, 1};
        const double qmax[3] = {3, 3, 3};

        tree.insert(min0, max0, 0);

        int found = -1;
        const int hits = tree.search(qmin, qmax, [&](int data) -> bool {
            found = data;
            return true;
        });
    ...
    MINI_TEST("SpatialRTree", "Search Miss") {
        ...
        const double min0[3] = {0, 0, 0};
        const double max0[3] = {1, 1, 1};
        const double qmin[3] = {2, 2, 2};
        const double qmax[3] = {3, 3, 3};

        tree.insert(min0, max0, 7);

        const int hits = tree.search(qmin, qmax, [](int) -> bool { return true; });
    ...
    MINI_TEST("SpatialRTree", "Remove") {
        ...
        tree.insert(min0, max0, 5);

        const bool removed = tree.remove(min0, max0, 5);
    ...
    MINI_TEST("SpatialRTree", "Search Count") {
        ...
        const double min1[3] = {2, 2, 2};
        const double max1[3] = {3, 3, 3};
        const double min2[3] = {3, 3, 3};
        const double max2[3] = {4, 4, 4};
        const double min3[3] = {10, 0, 0};
        const double max3[3] = {11, 1, 1};
        const double min4[3] = {0, 10, 0};
        const double max4[3] = {1, 11, 1};
        const double qmin[3] = {0, 0, 0};
        const double qmax[3] = {4, 4, 4};
    ...
    MINI_TEST("SpatialRTree", "Search Stop") {
        ...
        tree.insert(min0, max0, 0);
        tree.insert(min0, max0, 1);
        tree.insert(min0, max0, 2);

        const int hits = tree.search(min0, max0, [](int) -> bool { return false; });
    ...
    MINI_TEST("SpatialRTree", "Normalizes Reversed Bounds") {
        ...
        const double max0[3] = {-1, -2, -3};
        const double qmin[3] = {2, 3, 4};
        const double qmax[3] = {-2, -3, -4};

        tree.insert(min0, max0, 9);
        ...
        const bool removed = tree.remove(qmin, qmax, 9);

        MINI_CHECK(hits == 1);
        MINI_CHECK(found == std::vector<int>{9});
        MINI_CHECK(removed);
        MINI_CHECK(tree.count() == 0);
    }
    ...
    MINI_TEST("SpatialRTree", "Search 100 Boxes") {
        ...
        const double qmin[3] = {-60, -60, -60};
        const double qmax[3] = {60, 60, 60};

        for (const std::array<double, 6>& bounds : boxes)
            tree.insert(bounds.data(), bounds.data() + 3, tree.count());
