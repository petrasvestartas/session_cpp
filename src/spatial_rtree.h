...
private:
    ...
    bool search_internal(const Rect& rect, const Node* node, int& count, const std::function<bool(const DATATYPE&)>& callback) const {
        for (int i = 0; i < node->m_count; i++) {
            ...
            if (node->is_leaf()) {
                ...
                if (!callback(node->m_branch[i].m_data))
                    return false;
            } else if (!search_internal(rect, node->m_branch[i].m_child, count, callback)) {
                return false;
            }
        }
        return true;
    ...
    bool remove_rect_internal(const Rect& rect, const DATATYPE& data, Node* node, std::vector<Node*>& reinsert_list) {
        for (int i = 0; i < node->m_count; i++) {
            if (!overlaps(rect, node->m_branch[i].m_rect))
                continue;
            if (node->is_leaf()) {
                if (node->m_branch[i].m_data != data)
                    continue;
