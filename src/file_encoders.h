 ...
 */
inline void file_json_dump(const nlohmann::ordered_json &j,
 ...
 */
template <typename T>
inline auto file_json_loads(const std::string &json_str)
 ...
 */
template <typename T>
inline void file_json_dump(const T &obj, const std::string &filepath,
                      bool pretty = true) {
 ...
 */
template <typename T>
inline auto file_json_load(const std::string &filepath)
    -> decltype(T::jsonload(std::declval<nlohmann::json>())) {
  auto j = file_json_load_data(filepath);
  return T::jsonload(j);
}
