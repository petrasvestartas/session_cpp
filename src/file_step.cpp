...
struct StepEntity {
    std::vector<StepSubEntity> parts;
...
static void skip_ws(Cursor& c) {
    while (c.p < c.end && std::isspace((unsigned char)c.s[c.p])) c.p++;
}
...
static int parse_int(Cursor& c) {
    int id = 0;
    while (c.p < c.end && std::isdigit((unsigned char)c.s[c.p])) id = id * 10 + (c.s[c.p++] - '0');
    return id;
}

static double parse_number(Cursor& c) {
    const size_t start = c.p;
    if (c.p < c.end && (c.s[c.p] == '+' || c.s[c.p] == '-')) c.p++;
    while (c.p < c.end && (std::isdigit((unsigned char)c.s[c.p]) || c.s[c.p] == '.')) c.p++;
    if (c.p < c.end && (c.s[c.p] == 'e' || c.s[c.p] == 'E')) {
        size_t q = c.p + 1;
        if (q < c.end && (c.s[q] == '+' || c.s[q] == '-')) q++;
        if (q < c.end && std::isdigit((unsigned char)c.s[q])) {
            c.p = q;
            while (c.p < c.end && std::isdigit((unsigned char)c.s[c.p])) c.p++;
        }
    }
...
static std::string parse_ident(Cursor& c) {
    const size_t start = c.p;
    while (c.p < c.end && isident(c.s[c.p])) c.p++;
...
static StepParam parse_param(Cursor& c, int depth) {
    ...
    if (ch == '#') {
        ...
        r.tag = StepTag::List;
        if (depth < MAX_DEPTH) r.list = parse_params(c, depth + 1);
        else skip_list(c);
    } else if (ch == '\'') {
        r.tag = StepTag::Str;
        ...
        r.str = parse_ident(c);
        skip_ws(c);
        if (c.p < c.end && c.s[c.p] == '(') parse_params(c, depth + 1);
    } else {
        c.p++;
    }
    return r;
...
static StepSubEntity parse_sub_entity(Cursor& c) {
    ...
    skip_ws(c);
    if (c.p < c.end && c.s[c.p] == '(') sub.params = parse_params(c, 0);
    return sub;
}
...
static void parse_step_string(const std::string& content, StepFile& sf) {
    ...
    while (c.p < c.end) {
        ...
        if (c.s[c.p] == '(') {
            ...
            while (c.p < c.end) {
                skip_ws(c);
                if (c.p >= c.end || !std::isupper((unsigned char)c.s[c.p])) break;
...
static StepFile parse_step_file(const std::string& filepath) {
    ...
    if (lo == std::string::npos) return sf;
    const size_t semi = text.find(';', lo);
    if (semi == std::string::npos) return sf;
    const size_t endsec = text.find("ENDSEC", semi);
    if (endsec == std::string::npos) return sf;
...
static int first_ref(const std::vector<StepParam>& params) {
    for (const StepParam& p : params)
        if (p.tag == StepTag::Ref) return p.ref_id;
    return -1;
}
...
static std::vector<int> all_refs(const std::vector<StepParam>& params) {
    std::vector<int> out;
    for (const StepParam& p : params)
        if (p.tag == StepTag::Ref) out.push_back(p.ref_id);
    return out;
}
...
static std::vector<int> list_refs(const std::vector<StepParam>& params) {
    std::vector<int> out;
    for (const StepParam& p : params)
        for (int ref : all_refs(p.list)) out.push_back(ref);
    return out;
}
...
static std::vector<double> nums(const std::vector<StepParam>& params) {
    ...
    for (const StepParam& p : params)
        if (p.tag == StepTag::Num) out.push_back(p.num);
    return out;
}

static std::vector<int> int_list(const StepParam& p) {
    std::vector<int> out;
    for (double v : nums(p.list)) out.push_back((int)v);
    return out;
}
...
static std::vector<std::vector<double>> dbl_list_list(const StepParam& p) {
    ...
    for (const StepParam& row : p.list) out.push_back(dbl_list(row));
    return out;
}

static std::vector<std::vector<int>> ref_list_list(const StepParam& p) {
    std::vector<std::vector<int>> out;
    for (const StepParam& row : p.list) out.push_back(all_refs(row.list));
    return out;
}
...
static bool last_flag(const std::vector<StepParam>& params, bool fallback) {
    bool out = fallback;
    for (const StepParam& p : params)
        if (p.tag == StepTag::Enum) out = p.str == "T";
    return out;
}
...
struct Axis2 {
    ...
    Vector ax = Vector(1, 0, 0);
    Vector ay = Vector(0, 1, 0);
    Vector az = Vector(0, 0, 1);
    bool ok = false;
};
...
static std::tuple<double, double, bool> an_st_of(const AnFace& an, const Point& p) {
    ...
    if (an.kind == 2) return {s, z, radial_ok};
    if (an.kind == 3) {
        const double ca = std::cos(an.r2);
        return {s, std::abs(ca) > 1e-12 ? z / ca : z, radial_ok};
    }
    if (an.kind == 4) return {s, std::atan2(z, rho), radial_ok};
...
static Point an_eval(const AnFace& an, double s, double t) {
    ...
    if (an.kind == 2) return axis_point(an.a, an.radius * cs, an.radius * sn, t);
    if (an.kind == 3) {
    ...
    if (an.kind == 4) return axis_point(an.a, an.radius * ct * cs, an.radius * ct * sn, an.radius * st);
...
static NurbsSurface build_analytic_nurbs(const AnFace& an, int su0, int nsu, double t0, double t1, int sv0, int nsv) {
    ...
    if (an.kind == 2 || an.kind == 3) {
        ...
        for (int j = 0; j < 2; j++) {
            ...
            for (int i = 0; i < nu; i++) {
                ...
                if (!srf.set_cv_4d(i, j, cw[i] * p[0], cw[i] * p[1], cw[i] * p[2], cw[i])) return NurbsSurface();
            }
        }
        return srf;
    }
    ...
    for (int j = 0; j < nv; j++) {
        ...
        for (int i = 0; i < nu; i++) {
            ...
            if (!srf.set_cv_4d(i, j, wij * p[0], wij * p[1], wij * p[2], wij)) return NurbsSurface();
        }
    }
    return srf;
}
...
static NurbsSurface cylinder_surface(const Axis2& a, double radius, double u0, double u1, double v0, double v1) {
    ...
    for (int i = 0; i < n_u; i++)
        for (int j = 0; j < 2; j++) {
            ...
            if (!out.set_cv_4d(i, j, cw[i] * p[0], cw[i] * p[1], cw[i] * p[2], cw[i])) return NurbsSurface();
        }
    return out;
}
...
static std::pair<double, double> project(const Proj& pr, const Point& pt) {
    ...
    if (pr.kind == 1) return {d.dot(pr.a.ax), d.dot(pr.a.ay)};
...
static Proj bilinear_projector(const NurbsSurface& srf) {
    Proj pr;
    if (!srf.is_valid() || srf.degree(0) != 1 || srf.degree(1) != 1) return pr;
    ...
    if (eu2 <= 1e-28 || ev2 <= 1e-28) return pr;
    pr.kind = 1;
    pr.a.origin = p00;
    pr.a.ax = eu * (1.0 / eu2);
    pr.a.ay = ev * (1.0 / ev2);
...
static std::vector<Point> sample_nurbs(const NurbsCurve& nc, int n) {
    ...
    for (int i = 0; i < n; i++) pts.push_back(nc.point_at(n > 1 ? tmin + (tmax - tmin) * i / (n - 1) : tmin));
    return pts;
}
...
static NurbsCurve polyline_nurbs(const std::vector<Point>& pts, int dim) {
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (!nc.set_cv(i, pts[i])) return NurbsCurve();
    }
    return nc;
}
...
static NurbsCurve circle_nurbs(const Axis2& a, double rad, const Point& vs, const Point& ve) {
    ...
    if (vs.distance(ve) < 1e-10) ea = sa + 2.0 * PI;
    ...
    for (int i = 0; i < n_cp; i++) {
        ...
        if (!crv.set_cv_4d(i, w * p[0], w * p[1], w * p[2], w)) return NurbsCurve();
    }
    return crv;
}
...
static NurbsCurve exact_pcurve(const Proj& proj, const NurbsCurve& c3) {
    if (proj.kind != 1 || !c3.is_valid() || c3.cv_count() < 2) return NurbsCurve();
    ...
    for (int ci = 0; ci < c3.cv_count(); ci++) {
        ...
        if (std::abs(w) < 1e-300) return NurbsCurve();
        const auto [u, v] = project(proj, Point(wx / w, wy / w, wz / w));
        if (!p2.set_cv_4d(ci, u * w, v * w, 0.0, w)) return NurbsCurve();
    }
...
static void unwrap_seam(std::vector<Point>& uv) {
    for (size_t k = 1; k < uv.size(); k++) {
        ...
        if (du > 2.0) uv[k][0] -= 4.0;
        else if (du < -2.0) uv[k][0] += 4.0;
    }
}
...
static double grid_scale(const std::vector<Point>& grid) {
    double scale = 0.0;
    for (const Point& p : grid) scale = std::max(scale, p.distance(grid[0]));
    return scale;
}
...
static bool grid_closed(const std::vector<Point>& grid, int ns, double tol, bool along_u) {
    for (int k = 0; k < ns; k++) {
        ...
        if (a.distance(b) > tol) return false;
    }
    return true;
}
...
static bool grid_degenerate(const std::vector<Point>& grid, int ns, double tol, int j) {
    for (int k = 1; k < ns; k++)
        if (grid[k * ns + j].distance(grid[j]) > tol) return false;
    return true;
}
...
public:
    ...
    Axis2 get_axis2(int id) {
        ...
        if (refs.empty()) return ax_cache[id] = a;
        a.origin = get_point(refs[0]);
        a.az = refs.size() > 1 ? get_direction(refs[1]) : Vector(0, 0, 1);
        const double ln = a.az.magnitude();
        if (ln > 1e-12) a.az = a.az * (1.0 / ln);
        if (refs.size() > 2) a.ax = get_direction(refs[2]);
        else a.ax = std::abs(a.az[0]) < 0.9 ? Vector(1, 0, 0) : Vector(0, 1, 0);
        a.ax = a.ax - a.az * a.ax.dot(a.az);
        const double xn = a.ax.magnitude();
        if (xn > 1e-12) a.ax = a.ax * (1.0 / xn);
    ...
    NurbsCurve get_nurbs_curve(int id) {
        ...
        if (!base) {
            ...
            if (bp.size() < 2 || kp.size() < 2) return NurbsCurve();
            degree = (int)bp[0].num;
            pt_refs = all_refs(bp[1].list);
            mults = int_list(kp[0]);
            knots = dbl_list(kp[1]);
        }
        if (pt_refs.empty() || mults.empty() || knots.empty()) return NurbsCurve();
        ...
        const std::vector<double> weights = is_rat && !rat->params.empty() ? dbl_list(rat->params[0]) : std::vector<double>();
        NurbsCurve nc(3, is_rat, order, cv_count);
        if (nc.m_nurbsknot.size() != internal.size()) return NurbsCurve();
        nc.m_nurbsknot = internal;
        for (int i = 0; i < cv_count; i++) {
            const Point pt = get_point(pt_refs[i]);
            const double w = is_rat && i < (int)weights.size() ? weights[i] : 1.0;
            if (!nc.set_cv_4d(i, w * pt[0], w * pt[1], w * pt[2], w)) return NurbsCurve();
        }
        return nc;
    }
    ...
    NurbsSurface get_nurbs_surface(int id) {
        ...
        if (!base) {
            ...
            if (bp.size() < 3 || kp.size() < 4) return NurbsSurface();
            u_deg = (int)bp[0].num;
            v_deg = (int)bp[1].num;
        ...
        if (ctrl_pts.empty() || ctrl_pts[0].empty() || u_mults.empty() || v_mults.empty()) return NurbsSurface();
        ...
        if ((int)full_u.size() != cv_u + u_deg + 1 || (int)full_v.size() != cv_v + v_deg + 1) return NurbsSurface();
        const bool is_rat = rat != nullptr;
        const std::vector<std::vector<double>> weights = is_rat && !rat->params.empty() ? dbl_list_list(rat->params[0]) : std::vector<std::vector<double>>();
        ...
        for (int u = 0; u < cv_u; u++)
            for (int v = 0; v < cv_v && v < (int)ctrl_pts[u].size(); v++) {
                ...
                if (!srf.set_cv_4d(u, v, w * pt[0], w * pt[1], w * pt[2], w)) return NurbsSurface();
            }
    ...
    std::vector<Point> sample_curve(int curve_id, const Point& v_start, const Point& v_end, int n) {
        ...
        if (ax_ref < 0 || rad == 0 || !a.ok) return ends;
    ...
    NurbsSurface fill_surface(int id, double u0, double u1, double v0, double v1) {
        ...
        if (e->has("B_SPLINE_SURFACE_WITH_KNOTS")) return get_nurbs_surface(id);
        ...
        const Axis2 a = get_axis2(ax_ref);
        if (ax_ref < 0 || !a.ok) return NurbsSurface();
    ...
    AnFace get_analytic_srf(int id) {
        ...
        const StepEntity* e = get(id);
        if (!e) return an;
        const std::vector<std::string> kinds = {"CYLINDRICAL_SURFACE", "CONICAL_SURFACE", "SPHERICAL_SURFACE", "TOROIDAL_SURFACE"};
        for (size_t i = 0; i < kinds.size(); i++) {
            const StepSubEntity* sub = e->find(kinds[i]);
            if (!sub) continue;
            const int ax_ref = first_ref(sub->params);
            if (ax_ref < 0) return an;
            an.a = get_axis2(ax_ref);
            if (!an.a.ok) return an;
            const std::vector<double> rr = nums(sub->params);
            an.kind = (int)i + 2;
    ...
    std::vector<Point> pcurve_st_samples(int ec_geom_id, int surface_ref, bool forward_use, int n) {
        ...
        if (is_seam) sc = e->find("SEAM_CURVE");
        if (!sc) return {};
        std::vector<int> mine;
        for (int pid : list_refs(sc->params)) {
            ...
            if (refs.size() >= 2 && refs[0] == surface_ref) mine.push_back(refs[1]);
        }
...
static std::optional<Bound> bound_loop(const StepReader& r, int bid) {
    ...
    if (bent && !bsub) bsub = bent->find("FACE_BOUND");
    if (!bsub) return std::nullopt;
...
static void outer_first(std::vector<Loop>& loops) {
    ...
    for (const Loop& l : loops)
        if (l.is_outer) ordered.push_back(l);
    for (const Loop& l : loops)
        if (!l.is_outer) ordered.push_back(l);
    loops = ordered;
}
...
static std::pair<double, double> surface_periods(const Proj& proj, const NurbsSurface& proj_srf) {
    if (proj.kind == 2) return {4.0, 0.0};
    if (!proj_srf.is_valid()) return {0.0, 0.0};
...
static std::vector<Point> uv_of_samples(const Proj& proj, const NurbsSurface& proj_srf, const std::vector<Point>& samples) {
    std::vector<Point> uv;
    if (proj.kind != 0) {
    ...
    if (!proj_srf.is_valid() || samples.empty()) return {
        Point(0, 0, 0),
        Point(1, 0, 0)
    };
    const auto [du0, du1] = proj_srf.domain(0);
    ...
    for (size_t k = 0; k < samples.size(); k++) {
        ...
        if (k == 0) {
            ...
            if (d > 10 * d_ref + 1e-9) std::tie(u, v, d) = Closest::surface_point(proj_srf, samples[k]);
        }
        uv.emplace_back(u, v, 0.0);
        pu = u;
        pv = v;
...
static std::optional<Window> analytic_window(const std::vector<Loop>& loops, const AnFace& an) {
    ...
    w.t0 = tmin;
    w.t1 = tmax;
    if (w.nsu > 16) return std::nullopt;
    if (an.kind == 4 || an.kind == 5) {
        ...
        if (w.nsv > 16) return std::nullopt;
...
public:
    ...
    int vertex_at(const Point& q, double tol) {
        for (size_t i = 0; i < brep.m_vertices.size(); i++)
            if (brep.m_vertices[i].point.distance(q) <= tol) return (int)i;
    ...
    void attach_pcurve(int edge, int si, int c2, bool reversed_use) {
        for (BRepCurveOnSurface& pc : brep.m_edges[edge].pcurves) {
            if (pc.surface_index != si) continue;
            if (reversed_use) {
    ...
    void finish_face(int si, bool reversed_face, const std::vector<std::vector<PendingEdge>>& loops) {
        ...
        for (const std::vector<PendingEdge>& lp : loops) {
            ...
            for (const PendingEdge& pe : lp) {
                NurbsCurve c = pe.c2d;
                if (!pe.reversed || c.reverse()) attach_pcurve(pe.edge, si, brep.add_curve_2d(c), pe.reversed);
    ...
    NurbsCurve edge_curve(int curve_id, const Point& vs, const Point& ve) {
        ...
        if (e->has("B_SPLINE_CURVE_WITH_KNOTS")) return r.get_nurbs_curve(curve_id);
        const StepSubEntity* circle = e->find("CIRCLE");
        if (!circle) return NurbsCurve();
        const int ax_ref = first_ref(circle->params);
        const std::vector<double> rr = nums(circle->params);
        const double rad = rr.empty() ? 0.0 : rr[0];
        const Axis2 a = r.get_axis2(ax_ref);
        if (ax_ref < 0 || rad <= 0 || !a.ok) return NurbsCurve();
    ...
    int get_edge(int ec_id) {
        ...
        if (!crv3d.is_valid()) crv3d = polyline_nurbs(r.sample_curve(curve_id, vs, ve, 16), 3);
    ...
    std::vector<Point> st_projected(const AnFace& an, int geom_id, int edge_idx, bool rev, const Loop& lp) {
        ...
        std::vector<Point> ordered = r.sample_curve(geom_id, brep.m_vertices[be.start_vertex].point, brep.m_vertices[be.end_vertex].point, 48);
        ...
        for (size_t k = 0; k < ordered.size(); k++) {
            ...
            if (!ok && (k > 0 || have_prev)) s = k > 0 ? st.back()[0] : ps;
        ...
        if (rev) std::reverse(st.begin(), st.end());
        return st;
    }

    /// Loops of an analytic face with canonical (s, t) samples from the file pcurves or from projection
    bool analytic_loops(const std::vector<int>& bound_refs, int surface_ref, const AnFace& an, std::vector<Loop>& loops) {
        for (int bid : bound_refs) {
            ...
            for (int oe_id : b->oe_refs) {
                ...
                if (geom_id >= 0) le.uv = r.pcurve_st_samples(geom_id, surface_ref, oe_orient, 48);
                if (le.uv.size() < 2) {
                    lp.projected = true;
                    le.uv = st_projected(an, geom_id, edge_idx, le.reversed, lp);
                    if (le.uv.empty()) return false;
                }
                lp.edges.push_back(le);
            }
            if (!lp.edges.empty()) loops.push_back(lp);
    ...
    std::vector<PendingEdge> analytic_pending(const Loop& lp, const AnFace& an, const Window& w, double scale3) {
        ...
        for (const LoopEdge& le : lp.edges) {
            std::vector<Point> uv;
            for (const Point& p : le.uv) uv.push_back(chart_point(an, w, p));
            if (le.reversed) std::reverse(uv.begin(), uv.end());
            chains.push_back(uv);
        }
        ...
        for (size_t k = 0; k < lp.edges.size(); k++) {
            ...
            if (std::abs(a2[0] - b2[0]) + std::abs(a2[1] - b2[1]) <= 1e-7) continue;
            const Point p3a = chart_eval(an, w, a2);
            const Point p3b = chart_eval(an, w, b2);
            if (p3a.distance(p3b) >= scale3 * 1e-6) continue;
    ...
    bool add_face_analytic(const std::vector<int>& bound_refs, int surface_ref, bool same_sense, const AnFace& an) {
        ...
        if (!analytic_loops(bound_refs, surface_ref, an, loops)) return false;
        ...
        for (const Loop& lp : loops) pending.push_back(analytic_pending(lp, an, *w, scale3));
    ...
    bool add_face_vertex_loop(const std::vector<int>& vl_vertex_ids, int surface_ref, bool same_sense) {
        ...
        NurbsSurface srf;
        if (an.kind == 4) srf = build_analytic_nurbs(an, 0, 4, 0, 0, -1, 2);
        else if (an.kind == 5) srf = build_analytic_nurbs(an, 0, 4, 0, 0, 0, 4);
        ...
        if (!closed_u || !((degen_v0 && degen_v1) || closed_v)) return false;
    ...
    std::vector<int> vertex_loop_ids(const std::vector<int>& bound_refs) const {
        ...
        for (int bid : bound_refs) {
            ...
            if (!lent) continue;
            if (lent->has("EDGE_LOOP")) return {};
            const StepSubEntity* vl = lent->find("VERTEX_LOOP");
            const int ref = vl ? first_ref(vl->params) : -1;
            if (ref >= 0) ids.push_back(ref);
        }
        return ids;
    }

    /// Loops of a face on a projected surface: uv samples in curve order, exact pcurves under an affine projector
    void projected_loops(const std::vector<int>& bound_refs, const Proj& proj, const NurbsSurface& proj_srf, std::vector<Loop>& loops) {
        const int n = proj_srf.is_valid() ? 48 : 16;
        Proj exact;
        if (proj.kind == 1) exact = proj;
        else if (proj.kind == 0) exact = bilinear_projector(proj_srf);
        for (int bid : bound_refs) {
            const std::optional<Bound> b = bound_loop(r, bid);
            if (!b) continue;
            Loop lp;
            ...
            for (int oe_id : b->oe_refs) {
                ...
                if (proj.kind == 2) unwrap_seam(le.uv);
                if (be.curve_3d_index >= 0) le.pc2d = exact_pcurve(exact, brep.m_curves_3d[be.curve_3d_index]);
    ...
    void add_face(int face_id) {
        ...
        if (!vl_ids.empty() && add_face_vertex_loop(vl_ids, surface_ref, same_sense)) return;
        const AnFace an = r.get_analytic_srf(surface_ref);
        if (an.kind >= 2 && add_face_analytic(bound_refs, surface_ref, same_sense, an)) return;
        ...
        outer_first(loops);
        std::vector<std::vector<PendingEdge>> pending;
        for (const Loop& lp : loops) pending.push_back(pending_of(lp));
        finish_face(srf_idx, !same_sense, pending);
    }
    ...
    BRep build_from_shell(int shell_id) {
        ...
        if (sent && !shell) shell = sent->find("OPEN_SHELL");
        if (!shell) return BRep();
        brep.name = "step_brep";
        for (int f : list_refs(shell->params)) add_face(f);
        if (!face_refs.empty()) {
            const int sh = brep.add_shell(face_refs);
            if (sent->has("CLOSED_SHELL")) brep.add_solid({{sh, BRepOrientation::Forward}});
        }
...
static std::string fmt(double v) {
    char buf[64];
    if (std::abs(v) < 1e15 && v == (double)(long long)v) std::snprintf(buf, sizeof(buf), "%lld.", (long long)v);
    else std::snprintf(buf, sizeof(buf), "%.15g", v);
    std::string s = buf;
...
static std::string fmt_int_list(const std::vector<int>& items) {
    std::string s = "(";
    for (size_t i = 0; i < items.size(); i++) s += (i ? "," : "") + std::to_string(items[i]);
    return s + ")";
}

static std::string fmt_dbl_list(const std::vector<double>& items) {
    std::string s = "(";
    for (size_t i = 0; i < items.size(); i++) s += (i ? "," : "") + fmt(items[i]);
    return s + ")";
}
...
static std::string fmt_ref_grid(const std::vector<std::vector<int>>& rows) {
    std::string s = "(";
    for (size_t i = 0; i < rows.size(); i++) s += (i ? "," : "") + fmt_ref_list(rows[i]);
    return s + ")";
}

static std::string fmt_dbl_grid(const std::vector<std::vector<double>>& rows) {
    std::string s = "(";
    for (size_t i = 0; i < rows.size(); i++) s += (i ? "," : "") + fmt_dbl_list(rows[i]);
    return s + ")";
}
...
public:
    ...
    int write_nurbs_curve(const NurbsCurve& nc) {
        ...
        return write_raw(
            ...
            + "CURVE()GEOMETRIC_REPRESENTATION_ITEM()RATIONAL_B_SPLINE_CURVE(" + fmt_dbl_list(weights) + ")REPRESENTATION_ITEM(''))"
        );
    }
    ...
    int write_nurbs_surface(const NurbsSurface& srf) {
        ...
        for (int u = 0; u < cv_u; u++)
            for (int v = 0; v < cv_v; v++) {
                ...
                double z = 0.0;
                double w = 1.0;
                if (!srf.get_cv_4d(u, v, x, y, z, w)) return -1;
                if (std::abs(w) < 1e-14) w = 1.0;
        ...
        const std::string knots = fmt_int_list(ku_mults) + "," + fmt_int_list(kv_mults) + "," + fmt_dbl_list(ku_vals) + "," + fmt_dbl_list(kv_vals);
        if (!srf.is_rational())
            return write_raw(
                "B_SPLINE_SURFACE_WITH_KNOTS(''," + degrees + "," + fmt_ref_grid(pt_ids) + ",.UNSPECIFIED.,.F.,.F.,.U.,"
                + knots + ",.UNSPECIFIED.)"
            );
        return write_raw(
            "(BOUNDED_SURFACE()B_SPLINE_SURFACE(" + degrees + "," + fmt_ref_grid(pt_ids) + ",.UNSPECIFIED.,.F.,.F.,.U.)"
            + "B_SPLINE_SURFACE_WITH_KNOTS(" + knots + ",.UNSPECIFIED.)GEOMETRIC_REPRESENTATION_ITEM()"
            + "RATIONAL_B_SPLINE_SURFACE(" + fmt_dbl_grid(weight_grid) + ")REPRESENTATION_ITEM('')SURFACE())"
        );
    }
    ...
    int write_loop_as_face_bound(const NurbsSurfaceTrimmed& trimmed, const NurbsCurve& loop_2d, bool is_outer) {
        ...
        const int v0 = write_raw("VERTEX_POINT('',#" + std::to_string(write_point(pts3d[0][0], pts3d[0][1], pts3d[0][2])) + ")");
        const int crv3d = write_nurbs_curve(polyline_nurbs(pts3d, 3));
        if (crv3d < 0) return -1;
        write_nurbs_curve(loop_2d);
        const int ec = write_raw("EDGE_CURVE('',#" + std::to_string(v0) + ",#" + std::to_string(v0) + ",#" + std::to_string(crv3d) + ",.T.)");
        const int oe = write_raw("ORIENTED_EDGE('',*,*,#" + std::to_string(ec) + ",.T.)");
        const int el = write_raw("EDGE_LOOP('',(#" + std::to_string(oe) + "))");
        return write_raw(std::string(is_outer ? "FACE_OUTER_BOUND" : "FACE_BOUND") + "('',#" + std::to_string(el) + ",.T.)");
    }
    ...
    int write_body(const std::vector<int>& faces, bool closed) {
        ...
        const int shell = write_raw(std::string(closed ? "CLOSED_SHELL" : "OPEN_SHELL") + "(''," + fmt_ref_list(faces) + ")");
        if (closed) return write_raw("MANIFOLD_SOLID_BREP('',#" + std::to_string(shell) + ")");
    ...
    void finish_product(const std::vector<int>& bodies, bool closed, const std::string& name, double uncertainty = 1e-6, const std::vector<int>& styled_items = {}) {
        const int o = write_point(0, 0, 0);
        const int dz = write_raw("DIRECTION('',(0.,0.,1.))");
        const int dx = write_raw("DIRECTION('',(1.,0.,0.))");
        const int ax = write_raw("AXIS2_PLACEMENT_3D('',#" + std::to_string(o) + ",#" + std::to_string(dz) + ",#" + std::to_string(dx) + ")");
        ...
        const int un = write_raw("UNCERTAINTY_MEASURE_WITH_UNIT(LENGTH_MEASURE(" + fmt(uncertainty) + "),#" + std::to_string(lu) + ",'distance_accuracy_value','')");
        const int gc = write_raw(
            "(GEOMETRIC_REPRESENTATION_CONTEXT(3)GLOBAL_UNCERTAINTY_ASSIGNED_CONTEXT((#" + std::to_string(un) + "))GLOBAL_UNIT_ASSIGNED_CONTEXT((#"
            + std::to_string(lu) + ",#" + std::to_string(au) + ",#" + std::to_string(su) + "))REPRESENTATION_CONTEXT('',''))"
        );
        const int ac = write_raw("APPLICATION_CONTEXT('core data for automotive mechanical design processes')");
        write_raw("APPLICATION_PROTOCOL_DEFINITION('international standard','automotive_design',2000,#" + std::to_string(ac) + ")");
        ...
        const int pd = write_raw("PRODUCT_DEFINITION('design','',#" + std::to_string(pf) + ",#" + std::to_string(dc) + ")");
        ...
        if (bodies.empty()) rep_type = "SHAPE_REPRESENTATION";
        else if (closed) rep_type = "ADVANCED_BREP_SHAPE_REPRESENTATION";
        ...
        if (!styled_items.empty())
            write_raw("MECHANICAL_DESIGN_GEOMETRIC_PRESENTATION_REPRESENTATION(''," + fmt_ref_list(styled_items) + ",#" + std::to_string(gc) + ")");
    }
...
public:
    ...
    int edge_id(int ei) {
        ...
        return eid[ei] = w.write_raw("EDGE_CURVE('',#" + std::to_string(sv) + ",#" + std::to_string(ev) + ",#" + std::to_string(c) + ",.T.)");
    }
    ...
    int wire_id(const BRepRef& wire) {
        ...
        if (!oes.empty()) return w.write_raw("EDGE_LOOP(''," + fmt_ref_list(oes) + ")");
        if (any_vertex >= 0) return w.write_raw("VERTEX_LOOP('',#" + std::to_string(vertex_id(any_vertex)) + ")");
        return -1;
    }
    ...
    int face_id(int fi, BRepOrientation fo) {
        ...
        for (size_t wi = 0; wi < f.wires.size(); wi++) {
            ...
            if (loop < 0) continue;
            bounds.push_back(w.write_raw(std::string(wi == 0 ? "FACE_OUTER_BOUND" : "FACE_BOUND") + "('',#" + std::to_string(loop) + ",.T.)"));
        }
        if (bounds.empty()) return -1;
        const std::string sense = fo == BRepOrientation::Forward ? "T" : "F";
        return w.write_raw("ADVANCED_FACE(''," + fmt_ref_list(bounds) + ",#" + std::to_string(srf) + ",." + sense + ".)");
    }
};
...
static double vertex_diagonal(const BRep& brep) {
    if (brep.m_vertices.empty()) return 1.0;
...
std::vector<Point> read_file_step_points(const std::string& filepath) {
    ...
    for (int id : sf.ids_of_type("CARTESIAN_POINT")) out.push_back(r.get_point(id));
    return out;
}
...
std::vector<NurbsCurve> read_file_step_nurbscurves(const std::string& filepath) {
    ...
    for (int id : sf.ids_of_type("B_SPLINE_CURVE_WITH_KNOTS")) {
        ...
        if (nc.is_valid()) out.push_back(nc);
    }
    return out;
}
...
std::vector<NurbsSurface> read_file_step_nurbssurfaces(const std::string& filepath) {
    ...
    for (int id : sf.ids_of_type("B_SPLINE_SURFACE_WITH_KNOTS")) {
        ...
        if (srf.is_valid()) out.push_back(srf);
    }
    return out;
}
...
static NurbsCurve trimmed_outer_loop(StepReader& r, const std::vector<int>& bound_refs) {
    for (int bid : bound_refs) {
        ...
        for (int oe_id : b->oe_refs) {
            ...
            if (ecr.size() < 3) continue;
            const Point vs = r.get_point(ecr[0]);
            const Point ve = r.get_point(ecr[1]);
            for (const Point& s : r.sample_curve(ecr[2], vs, ve, 8)) uv_pts.emplace_back(s[0], s[1], 0.0);
        }
...
std::vector<NurbsSurfaceTrimmed> read_file_step_nurbssurfaces_trimmed(const std::string& filepath) {
    ...
    for (int face_id : sf.ids_of_type("ADVANCED_FACE")) {
        ...
        if (!surf || !surf->has("B_SPLINE_SURFACE_WITH_KNOTS")) continue;
        ...
        if (nst.m_surface.is_valid() && nst.m_outer_loop.is_valid()) out.push_back(nst);
    }
    return out;
}
...
std::vector<BRep> read_file_step_breps(const std::string& filepath) {
    ...
    for (int id : ids) {
        ...
        if (!root) root = e->find("BREP_WITH_VOIDS");
        if (!root) root = e->find("SHELL_BASED_SURFACE_MODEL");
        if (!root) continue;
        for (int sh : all_refs(root->params)) shell_refs.push_back(sh);
        for (int sh : list_refs(root->params)) shell_refs.push_back(sh);
    }
    std::vector<BRep> out;
...
void write_file_step_nurbscurves(const std::vector<NurbsCurve>& curves, const std::string& filepath) {
    StepWriter w;
    for (const NurbsCurve& nc : curves) w.write_nurbs_curve(nc);
...
void write_file_step_nurbssurfaces(const std::vector<NurbsSurface>& surfaces, const std::string& filepath) {
    StepWriter w;
    for (const NurbsSurface& srf : surfaces) w.write_nurbs_surface(srf);
    write_step_string(w.emit(), filepath);
}
...
void write_file_step_nurbssurfaces_trimmed(const std::vector<NurbsSurfaceTrimmed>& trimmed, const std::string& filepath) {
    ...
    if (body >= 0) bodies.push_back(body);
...
void write_file_step_breps(const std::vector<const BRep*>& breps, const std::string& name, const std::string& filepath) {
    ...
    for (const BRep* b : breps) {
        ...
        for (const auto& [ids, closed] : groups) {
            ...
            if (body >= 0) bodies.push_back(body);
            any_closed = any_closed || closed;
            for (int fid : ids) styled.push_back(w.write_raw("STYLED_ITEM('',(#" + std::to_string(psa) + "),#" + std::to_string(fid) + ")"));
        }
    }
