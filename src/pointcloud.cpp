...
PointCloud::PointCloud(const std::vector<Point>& points, const std::vector<Vector>& normals, const std::vector<Color>& colors) {
    ...
    for (const Vector& normal : normals)
        add_normal(normal);
...
void PointCloud::lod_store_nodes(const SpatialOctree& tree) {
    ...
    for (int i = 0; i < tree.node_count(); i++) {
        ...
        const std::pair<int, int> span = tree.node_range(i);
        ...
        _lod_first.push_back(span.first);
        _lod_count.push_back(span.second);
...
std::pair<Point, double> PointCloud::lod_cube(int i) const {
    ...
    const Point corner(_lod_min[i * 3], _lod_min[i * 3 + 1], _lod_min[i * 3 + 2]);

    return {corner + Vector(half, half, half), _lod_size[i]};
}
...
// Protobuf
// ═══════════════════════════════════════════════════════════════════════════
session_proto::PointCloud PointCloud::to_proto() const {
    ...
    proto.mutable_normals()->Add(_normals.begin(), _normals.end());
    proto.mutable_lod_min()->Add(_lod_min.begin(), _lod_min.end());
    proto.mutable_lod_size()->Add(_lod_size.begin(), _lod_size.end());
    proto.mutable_lod_spacing()->Add(_lod_spacing.begin(), _lod_spacing.end());
    proto.mutable_lod_level()->Add(_lod_level.begin(), _lod_level.end());
    proto.mutable_lod_first()->Add(_lod_first.begin(), _lod_first.end());
    proto.mutable_lod_count()->Add(_lod_count.begin(), _lod_count.end());
    proto.mutable_lod_children()->Add(_lod_children.begin(), _lod_children.end());
    proto.mutable_point_ids()->Add(_point_ids.begin(), _point_ids.end());

    return proto;
}

PointCloud PointCloud::from_proto(const session_proto::PointCloud& proto) {

    PointCloud cloud = from_coords(
        std::vector<double>(proto.coords().begin(), proto.coords().end()),
        std::vector<int>(proto.colors().begin(), proto.colors().end()),
        std::vector<double>(proto.normals().begin(), proto.normals().end())
    );
...
PointCloud PointCloud::pb_loads(const std::string& data) {
    ...
    if (!proto.ParseFromString(data))
        throw std::runtime_error("Failed to parse PointCloud protobuf data");
