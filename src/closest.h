...
public:
  ...
  /// Parameters and distance of the closest approach between two curves
  static std::tuple<double, double, double> curve_curve(
    const NurbsCurve& curve0,
    const NurbsCurve& curve1
  );

  /// Closest point, parameter in [0, 1] and distance on a segment
  static std::tuple<Point, double, double> line_point(
    const Line& line,
    const Point& test_point
  );

  /// Closest point, length parameter in [0, 1] and distance on a polyline
  ...
  /// Closest point, face key and distance on a mesh via its triangle BVH
  static std::tuple<Point, size_t, double> mesh_point(
    const Mesh& mesh,
    const Point& test_point
  );

  /// Closest point, face key and distance on a mesh via its triangle AABB tree
  static std::tuple<Point, size_t, double> mesh_point_aabb(
    const Mesh& mesh,
    const Point& test_point
  );

  /// Closest point, index and distance in a cloud by linear scan
  static std::tuple<Point, size_t, double> pointcloud_point(
    const PointCloud& cloud,
    const Point& test_point
  );

  /// Closest point, index and distance in a cloud via a kd-tree
