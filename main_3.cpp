...
int main() {
    ...
    // // Single-curved triangle — degree-2 in U (parabolic base), degree-1 in V (linear to apex)
    // {
    //     NurbsSurface s = NurbsSurface::create(false, false, 2, 1, 3, 2, {
    //         Point(0,0,0), Point(2,4,0),
    //         Point(2,0,3), Point(2,4,0),
    //         Point(4,0,0), Point(2,4,0),
    //     });
    //     Mesh m = RemeshNurbsSurfaceAdaptive(s).mesh();

    //     bool is_valid = m.is_valid();
    //     std::cout << is_valid << " Single-Curved Triangle: is_valid=" << is_valid << ", vertex_count=" << m.number_of_vertices() << "\n";
    ...
    // Double-curved triangle — degree-2 in U and V, singular apex at v=max
    {
        ...
        session.add_mesh(std::make_shared<Mesh>(std::move(m)));
        session.add_nurbssurface(std::make_shared<NurbsSurface>(std::move(s)));
    }

    // Double-curved triangle 2 — narrowing base, curved ridge, singular apex
    {
        NurbsSurface s = NurbsSurface::create(false, false, 2, 2, 3, 3, {
            Point(4,0,0), Point(2,0,1.5), Point(0,0,0),
            Point(3,2,0), Point(2,2,1.5), Point(1,2,0),
            Point(2,4,0), Point(2,4,0),   Point(2,4,0),
        });
        Mesh m = RemeshNurbsSurfaceAdaptive(s).mesh();

        bool is_valid = m.is_valid();
        std::cout << is_valid << " Double-Curved Triangle 2: is_valid=" << is_valid << ", vertex_count=" << m.number_of_vertices() << "\n";
